...
export const BrowserFeature: React.FC = () => {
  ...
  const handleReload = async () => {
    if (!workspaceId) {
      toast({
        title: t('workspace.preview.browser.restart.failed'),
        description: t('workspace.preview.browser.error.noWorkspace'),
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      ...
      toast({
        title: t('workspace.preview.browser.restart.started'),
        description: t('workspace.preview.browser.restart.description'),
      });
  ...
  return (
    ...
    >
      <FeatureHeader
        title={t('workspace.preview.browser.title')}
        icon={Globe}
        actions={
          <div className="flex items-center gap-1">
            {(isConnected || isRestarting || browserStatus === 'restarting') && (
              <Button
                ...
                title={t('workspace.preview.browser.actions.restartContainer')}
              >
                ...
                {t('workspace.preview.browser.actions.restartContainer')}
              </Button>
            )}
            <Button
              variant="ghost"
          ...
          <div className="absolute inset-0 flex items-center justify-center bg-background/80 backdrop-blur-sm z-10">
            <div className="text-center space-y-4">
              ...
              <div className="text-sm text-muted-foreground">
                {t('workspace.preview.browser.connecting')}
              </div>
            </div>
          </div>
        )}
          ...
          <div className="absolute inset-0 flex items-center justify-center bg-background">
            <div className="text-center space-y-4 max-w-md">
              ...
              <div className="text-lg font-semibold">
                {t('workspace.preview.browser.restart.inProgress')}
              </div>
              <div className="text-sm text-muted-foreground">
                {t('workspace.preview.browser.restart.description')}
              </div>
            </div>
          </div>
        )}
        ...
        {!shouldShowStream && browserStatus !== 'restarting' && (
          <div className="absolute inset-0 flex items-center justify-center bg-background">
            <div className="text-center space-y-4 max-w-md">
              ...
              <div className="text-lg font-semibold">
                {t('workspace.preview.browser.notReady.title')}
              </div>
              <div className="text-sm text-muted-foreground">
                {t('workspace.preview.browser.notReady.description')}
              </div>
              {browserStatus === 'stopped' && (
                <div className="text-xs text-muted-foreground mt-4">
                  <div className="bg-muted p-3 rounded-md text-left">
                    <div className="font-mono text-xs">
                      {t('workspace.preview.browser.notReady.hint')}
                    </div>
                  </div>
                </div>
              )}
        ...
        {streamError && connectionState === 'failed' && (
          <div className="absolute inset-0 flex items-center justify-center bg-background">
            <div className="text-center space-y-4 max-w-md">
              ...
              <div className="text-lg font-semibold">
                {t('workspace.preview.browser.error.connectionFailed')}
              </div>
              <div className="text-sm text-muted-foreground">{streamError}</div>
              <Button onClick={reconnect}>
                <RotateCw className="h-4 w-4 mr-2" />
                {t('workspace.preview.browser.actions.retry')}
              </Button>
            </div>
          </div>
        )}
