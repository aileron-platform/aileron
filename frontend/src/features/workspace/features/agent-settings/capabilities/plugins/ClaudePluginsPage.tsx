...
import { AgentSettingsLayerSelector, AgentSettingsSourceFilter } from '../../components/AgentSettingsSourceControls';
import { DetailSection } from '../../components/plugin-list/DetailSection';
...
const resourceKeys: ClaudeResourceKey[] = [
  'commands',
  'agents',
  'hooks',
  'mcpServers',
...
const resourceSettings: Record<ClaudeResourceKey, PluginSettingsResourceKind> = {
...
const resourceIcons: Record<ClaudeResourceKey, LucideIcon> = {
  commands: TerminalSquare,
  agents: Package,
  hooks: Wrench,
  mcpServers: Server,
  skills: Sparkles,
  outputStyles: Sparkles,
};
...
const ClaudePluginsPage: React.FC = () => {
  ...
  const api = useMemo(() => createAgentSettingsApi('claude-code'), []);
  ...
  const visiblePlugins = useMemo(() => filterClaudePlugins(plugins, {
    displayMode,
    searchQuery,
    marketplaceFilter,
    categoryFilter,
  ...
  const pagination = getClaudePluginPagination(visiblePlugins.length, currentPage, PLUGINS_PER_PAGE);
  ...
  return (
      ...
      headerActions={
        <div className="flex flex-wrap items-center gap-2">
          <AgentSettingsLayerSelector
            value={scope}
            onChange={value => setScope(value as ClaudeScopeFilter)}
    ...
    >
          ...
          <PluginCardGrid>
            {paginatedPlugins.map((plugin) => (
              <ClaudePluginCard
                ...
                onToggle={() => toggleMutation.mutate({ pluginId: plugin.id, nextEnabled: getNextClaudeScopeEnabled(plugin, scope) })}
              />
            ))}
          </PluginCardGrid>
          <PluginPagination
      ...
      <PluginDetailDialog
        ...
        icon={Package}
      >
        <ClaudePluginDetailPanel
          detail={detail}
          loading={detailQuery.isLoading}
          workspaceId={workspaceId || ''}
          openResources={Boolean(deepLinkedResource)}
        />
      </PluginDetailDialog>
    </SettingsWorkflowShell>
  );
...
const ClaudePluginCard: React.FC<{
  ...
  onDetails: () => void;
  onToggle: () => void;
}> = ({ plugin, pending, readOnly, scope, onDetails, onToggle }) => {
  const { t } = useI18n();
  const description = plugin.description?.trim() || t(`${I18N_PREFIX}.descriptionFallback`);
  const toggleDisabled = pending || scope === 'all';
  const selectedScopeEnabled = scope !== 'all' && getClaudeScopeInstallation(plugin, scope)?.enabled === true;
    ...
    >
      <div className="flex flex-wrap gap-2">
        {plugin.installations.map((installation) => (
          <Badge key={installation.scope} variant="outline">
...
const ClaudePluginDetailPanel: React.FC<{
  ...
  return (
    ...
    >
      ...
      <TabsContent value="resources" className="mt-0 min-h-0 flex-1 overflow-auto px-6 pb-6 pt-5">
        <ClaudePluginResources detail={detail} workspaceId={workspaceId} />
      </TabsContent>
          ...
          {detail.installations.map((installation) => (
            ...
            >
              ...
              <span className="min-w-0 truncate text-muted-foreground">
                {installation.version ?? t(`${I18N_PREFIX}.detail.versionUnknown`)}
              </span>
...
const ClaudePluginResources: React.FC<{
  detail: ClaudePluginDetail;
  workspaceId: string;
}> = ({ detail, workspaceId }) => {
  const { t } = useI18n();
    ...
    <div className="grid gap-2 sm:grid-cols-2">
      {visibleKeys.map((key) => (
        <ResourceSummary
          ...
          href={buildPluginResourceSettingsHref({
            workspaceId,
            provider: 'claude-code',
            resource: resourceSettings[key],
            pluginId: detail.id,
          })}
          linkLabel={t(`${I18N_PREFIX}.detail.openResource`, {
            resource: t(`${I18N_PREFIX}.detail.${key}`),
          })}
        />
      ))}
    </div>
