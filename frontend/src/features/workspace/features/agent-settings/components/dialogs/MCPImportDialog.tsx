...
interface MCPImportDialogProps {
  open: boolean;
  onClose: () => void;
  onImport: (options: { scope: McpImportScope; file: File; overwrite?: boolean }) => Promise<McpImportResult>;
...
const MCPImportDialog: React.FC<MCPImportDialogProps> = ({ open, onClose, onImport, availableScopes, i18nNamespace = 'workspace.agentSettings.common' }) => {
  const importScopes = getMcpImportScopes(availableScopes);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [importResult, setImportResult] = useState<McpImportResult | null>(null);
  const [selectedFile, setSelectedFile] = useState<File | null>(null);
  const [isDragOver, setIsDragOver] = useState(false);
  const [formData, setFormData] = useState<{ scope: McpImportScope }>({ scope: importScopes[0] ?? 'project' });
  ...
  const handleFileSelect = (file: File) => {
    const validationError = validateMcpImportFile(file);
    if (validationError) {
      setError(t(`${i18nNamespace}.mcp.dialogs.import.errors.${validationError}`));
      return;
    }
  ...
  const resultEntries = importResult ? buildMcpImportResultEntries(importResult) : [];
            ...
            <>
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value: McpImportScope) =>
