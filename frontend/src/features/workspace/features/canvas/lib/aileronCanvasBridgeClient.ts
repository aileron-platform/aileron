...
import type { CanvasReviewTarget } from '../../../api/workspaceRuntimeApi';
...
export type AileronCanvasBridgeHandlers = {
  ...
  onSkillEvent?: (eventType: string, data: unknown) => void;
};
  ...
  return (event: MessageEvent) => {
    ...
    switch (event.data.type) {
      ...
      case 'SKILL_EVENT': {
        ...
        if (!eventType) return;
        handlers.onSkillEvent?.(eventType, event.data.payload?.data);
        return;
      }
      default:
