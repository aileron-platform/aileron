...
import {
  getGlobalNavigationModule,
  type GlobalNavigationModule,
} from './globalNavigationModel';
...
export const GlobalNavigation: React.FC = () => {
  ...
  const hasPlatformAccess = platformRole !== null;
  const canReadPlatformResources = hasPlatformOperation(
    OPERATION_IDS.platformResourcesRead,
  );
  const { toast } = useToast();
  const currentModule = React.useMemo(
    () => getGlobalNavigationModule(location.pathname),
    [location.pathname],
  );
  ...
  const userDisplayName = React.useMemo(() => {
    ...
    return user.display_name || user.username || user.email || t('navigation.userMenu.defaultUser');
  }, [t, user]);
  ...
  const getProvisionerLabel = React.useCallback(
    (provisioner?: WorkspaceListItem['provisioner']) => {
  ...
  const setActiveModule = (module: GlobalNavigationModule) => {
    switch (module) {
      case 'workspace':
      ...
      case 'user-management':
        navigate(ROUTES.userManagement.root);
        break;
      case 'platform-resources':
        navigate(ROUTES.platformResources.workspaces);
        break;
      default:
        navigate(ROUTES.workspace.root);
    }
  ...
  const handleWorkspaceSelect = (workspaceId: string) => {
    ...
    navigate(
      permissions.canUseChat
        ? ROUTES.workspace.home(workspaceId)
        : ROUTES.workspace.files(workspaceId),
    );
  };
  ...
  const shouldShowWorkspaceSelector = React.useMemo(() => {
    ...
    return hasPlatformAccess
      ...
      && !isProfilePage
      && !isSettingsPage;
  }, [currentModule, hasPlatformAccess, location.pathname]);
  ...
  return (
    <header className="h-10 border-b border-border/50 bg-gradient-to-r from-card to-card/95 backdrop-blur-md px-4 flex items-center justify-between shadow-sm">
      <div className="flex items-center gap-3">
        <h1 className="text-base font-bold cursor-pointer flex items-center gap-2 hover:opacity-80 transition-all duration-300 group" onClick={() => navigate(ROUTES.workspace.root)}>
                      ...
                      return (
                        <DropdownMenuItem
                          ...
                          className={cn(
                            ...
                            workspace.id === selectedWorkspaceId && 'bg-accent text-accent-foreground'
                          )}
                        >
      ...
      <div className="flex items-center gap-3">
        <div className="flex items-center gap-2">
          <div
            data-testid="global-navigation-workspace-group"
            className="flex items-center gap-0.5 bg-muted/50 rounded-md p-0.5"
          >
            {hasPlatformAccess ? (
              <Button
                variant={currentModule === 'workspace' ? 'default' : 'ghost'}
                size="sm"
          ...
          <div
            data-testid="global-navigation-application-group"
            className="flex items-center gap-0.5 bg-muted/50 rounded-md p-0.5"
          >
            {hasPlatformAccess ? (
              <Button
                variant={currentModule === 'automation' ? 'default' : 'ghost'}
                size="sm"
                className={`gap-1 transition-all duration-300 h-7 px-2 text-xs ${
            ...
            {hasPlatformAccess ? (
              <Button
                variant={currentModule === 'marketplace' ? 'default' : 'ghost'}
                size="sm"
                className={`gap-1 transition-all duration-300 h-7 px-2 text-xs ${
            ...
            {hasPlatformAccess ? (
              <Button
                variant={currentModule === 'knowledge-base' ? 'default' : 'ghost'}
                size="sm"
                className={`gap-1 transition-all duration-300 h-7 px-2 text-xs ${
          ...
          {isPlatformAdmin || canReadPlatformResources ? (
            <div
              data-testid="global-navigation-administration-group"
              className="flex items-center gap-0.5 bg-muted/50 rounded-md p-0.5"
            >
              {canReadPlatformResources ? (
                <Button
                  variant={currentModule === 'platform-resources' ? 'default' : 'ghost'}
                  size="sm"
                  className={`gap-1 transition-all duration-300 h-7 px-2 text-xs ${
                    currentModule === 'platform-resources'
                      ? 'bg-primary text-primary-foreground shadow-sm'
                      : 'hover:bg-muted text-muted-foreground hover:text-foreground'
                  }`}
                  onClick={() => setActiveModule('platform-resources')}
                >
                  <Database className="h-3 w-3" />
                  {t('platformResources.navigation')}
                </Button>
              ) : null}
