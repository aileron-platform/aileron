...
describe('shared slashCommandApi.listPickerItems', () => {
  beforeEach(() => {
    getMock.mockReset();
  });
  ...
  it('maps flat Runtime slash command responses into picker items', async () => {
    ...
    const items = await slashCommandApi.list('http://runtime.test', 'ws-1', 'claude-code');

    expect(items).toEqual([
      expect.objectContaining({
        kind: 'slash-command',
        scope: 'project',
        fileName: 'opsx/apply.md',
        displayName: 'opsx/apply',
        invocation: '/opsx/apply',
        description: 'Implement tasks from an OpenSpec change',
      }),
    ]);
  });
