...
interface FlatSlashCommandCollectionResponse {
  workspaceId: string;
  items: SlashCommandSummaryResponse[];
...
type SlashCommandListResponse = SlashCommandCollectionResponse | FlatSlashCommandCollectionResponse;
...
const mapSummaryToItem = (
  ...
  const fileName = summary.fileName ?? summary.path;
  if (!fileName) {
    throw new Error('Slash command summary is missing fileName or path.');
  }

  // Build display names for plugin and non-plugin command scopes.
  const displayName = buildSlashCommandDisplayName(fileName, namespace, pluginName);
  const category = buildSlashCommandCategory(scope, namespace);
  const id = namespace ? `${scope}:${namespace}:${fileName}` : `${scope}:${fileName}`;
...
const mapSlashCommandListResponse = (response: SlashCommandListResponse): SlashCommandItem[] => {
  if ('items' in response) {
    return response.items.map((document) => mapSummaryToItem(document.scope, document));
  }

  return response.scopes.flatMap(({ scope, documents }) =>
...
const mapSkillNodeToItem = (
  ...
  return {
    ...
    invocation: buildSkillInvocation(apiPrefix, skillName),
    tags: [],
  };
};
...
const mapPluginSkillToItem = (
  ...
  description: '',
  invocation: buildSkillInvocation(apiPrefix, skill.skillName, skill.pluginName),
  tags: [],
});
...
const mapCodexPluginSkillToItem = (
  ...
  return {
    ...
    invocation: buildSkillInvocation('codex', skillName, pluginName),
    tags: [],
  };
};
...
export const slashCommandApi = {
  async list(
    ...
    const response = await client.get<SlashCommandListResponse>(path);
    const items = mapSlashCommandListResponse(response);
    ...
    for (const scope of scopes) {
      try {
        if (scope === 'plugin') {
          ...
          skillItems.push(
            ...pluginSkills.plugins.map((skill) => mapPluginSkillToItem(scope, skill, apiPrefix)),
          );
          continue;
        }
        ...
        skillItems.push(...skillNodes.map((node) => mapSkillNodeToItem(node, scope, apiPrefix)));
      } catch (error) {
