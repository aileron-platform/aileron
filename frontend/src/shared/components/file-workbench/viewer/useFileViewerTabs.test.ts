...
describe('useFileViewerTabs', () => {
  ...
  it('replaces the content of an existing tab and discards its draft', () => {
    const { result } = renderHook(() => useFileViewerTabs());

    act(() => {
      result.current.openFile(buildNode('/pkg/a.md'), 'draft');
    });
    act(() => {
      result.current.applyTabsChange([
        { ...result.current.tabs[0], content: 'draft', isModified: true },
      ]);
    });
    act(() => {
      result.current.replaceFileContent('/pkg/a.md', 'backend');
    });

    expect(result.current.tabs[0]).toMatchObject({
      content: 'backend',
      originalContent: 'backend',
      isModified: false,
    });
  });
