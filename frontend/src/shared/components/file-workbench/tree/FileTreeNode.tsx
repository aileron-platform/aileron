...
import { TreeNodeRow } from '../primitives/TreeNodeRow';
import {
...
import { getFileExtension } from '../model/fileTreeModel';
...
export const FileTreeNode: React.FC<FileTreeNodeProps> = ({
  ...
  const handleDragStart = (event: React.DragEvent) => {
    ...
    if (node.type === 'file') {
      ...
      event.dataTransfer.setData(WORKSPACE_FILE_REFERENCE_MIME, toWorkspaceFileReferencePath(node.path));
    } else {
          ...
          <span className="flex max-w-40 items-center gap-1 overflow-hidden">
            {node.badges.map(badge => (
              <Badge
                ...
                variant="outline"
                className={cn(
                  'max-w-24 truncate px-1 py-0 text-[9px]',
                  treeBadgeToneClassName[badge.tone ?? 'default'],
                )}
                title={badge.title}
              >
                {badge.label}
