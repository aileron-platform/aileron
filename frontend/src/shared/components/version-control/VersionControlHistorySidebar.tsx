...
import { Clock, Copy, Eye, FileText, GitCommit, Loader2, RotateCcw, Search, User } from 'lucide-react';
...
interface VersionControlHistorySidebarProps {
  ...
  onRevertCommit?: (commit: VersionControlCommitSummary) => void;
...
export const VersionControlHistorySidebar: React.FC<VersionControlHistorySidebarProps> = ({
  ...
  const renderCommitList = (
    <div className="flex h-full min-w-0 max-w-full flex-col overflow-hidden">
      <div className="flex h-10 flex-shrink-0 items-center justify-between gap-2 border-b border-border bg-muted/30 px-3">
      ...
      {commits.length === 0 ? (
        <EmptyState
          className="flex-1"
          icon={GitCommit}
          title={t('shared.versionControl.commitHistory.empty')}
        />
      ) : (
                ...
                >
                  <div className="px-2 py-1">
                    <ContextMenu>
                      ...
                      <ContextMenuContent>
                        <ContextMenuItem onSelect={() => onCommitSelect(commit)}>
                          <Eye className="h-3.5 w-3.5" />
                          {t('shared.versionControl.commit.actions.view')}
                        </ContextMenuItem>
                        <ContextMenuItem onSelect={() => {
                          if (onCopySha) onCopySha(commit.id);
                          else void navigator.clipboard?.writeText(commit.id);
                        }}>
                          <Copy className="h-3.5 w-3.5" />
                          {t('shared.versionControl.commit.actions.copySha')}
                        </ContextMenuItem>
                        <ContextMenuSeparator />
                        <ContextMenuItem
                          disabled={mutationDisabled || !onRevertCommit}
                          onSelect={() => onRevertCommit?.(commit)}
                        >
                          <RotateCcw className="h-3.5 w-3.5" />
                          {t('shared.versionControl.commit.actions.revert')}
                        </ContextMenuItem>
                      </ContextMenuContent>
                    </ContextMenu>
                  </div>
