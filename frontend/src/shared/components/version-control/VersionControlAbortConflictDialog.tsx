...
interface VersionControlAbortConflictDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onConfirm: () => Promise<void>;
}

export const VersionControlAbortConflictDialog: React.FC<VersionControlAbortConflictDialogProps> = ({
  open,
  onOpenChange,
  onConfirm,
}) => {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      setError(abortError instanceof Error
        ? abortError.message
        : t('shared.versionControl.conflictResolution.abortDialog.error'));
    } finally {
      setIsPending(false);
    }
  };
      ...
      >
        <DialogHeader className="shrink-0">
          <DialogHeading icon={AlertTriangle} iconClassName="h-4 w-4 text-destructive">
            {t('shared.versionControl.conflictResolution.abortDialog.title')}
          </DialogHeading>
          <DialogDescription>
            {t('shared.versionControl.conflictResolution.abortDialog.description')}
          </DialogDescription>
        </DialogHeader>
        {error && (
        ...
        <DialogFooter className="shrink-0 sm:justify-between sm:space-x-0">
          <Button type="button" variant="outline" disabled={isPending} onClick={() => handleOpenChange(false)}>
            {t('shared.versionControl.conflictResolution.abortDialog.cancel')}
          </Button>
          <Button type="button" variant="destructive" disabled={isPending} onClick={() => void handleConfirm()}>
            {isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
            {isPending
              ? t('shared.versionControl.conflictResolution.abortDialog.pending')
              : t('shared.versionControl.conflictResolution.abortDialog.confirm')}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
