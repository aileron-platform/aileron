/**
 * JobExecutionsDialog - 共用的任務執行記錄對話框
 * 可在多個功能模組中重複使用，提供統一的任務執行記錄查看體驗
 */
...
interface JobExecutionsDialogProps {
  ...
  onClose: () => void;
  onRefresh?: () => void;  // 重新載入執行記錄
  title?: string;
  description?: string;
  runtimeBaseUrl?: string;  // 用於查看對話
}
...
export const JobExecutionsDialog: React.FC<JobExecutionsDialogProps> = ({
  ...
  // 自動輪詢：當對話框開啟且有活躍任務時，每 3 秒自動重新整理
  useEffect(() => {
    if (!isOpen || !onRefresh) {
      return;
    }

    // 檢查是否有活躍的執行任務（running, queued, waiting）
