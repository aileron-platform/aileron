  ...
  return (
    <div className={cn('rounded-lg border p-4 space-y-3', headerStyle())}>
      ...
      <div className="rounded-md bg-background/60 p-3 text-xs text-muted-foreground">
        <div className="font-medium text-foreground mb-2">工具輸入</div>
        {hasDisplayToolInput ? (
          <pre className="whitespace-pre-wrap break-words">{JSON.stringify(displayToolInput, null, 2)}</pre>
        ) : (
          <div className="text-muted-foreground">未提供參數</div>
        )}
      </div>
      ...
      {decisionType === 'user_input' && (
        <div className="space-y-2">
          {isPending ? (
            ...
            <div className="text-sm text-muted-foreground">{content || reason || '已完成'}</div>
          )}
        </div>
      )}
      ...
      {isWaiting && (
        <div className="text-xs text-muted-foreground">等待前一個決策完成</div>
      )}
    </div>
  );
};
