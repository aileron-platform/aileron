...
import type { HookActionConfig, HookType } from './model/hookTypes';

export interface HookActionEditorLabels {
  ...
  executionArgsLabel?: string;
  executionArgsPlaceholder?: string;
  executionArgsHelp?: string;
  executionAdditionalContextLimitLabel?: string;
  executionAdditionalContextLimitPlaceholder?: string;
  executionAdditionalContextLimitHelp?: string;
  executionCommandWindowsLabel?: string;
  executionCommandWindowsPlaceholder?: string;
  executionCommandWindowsHelp?: string;
      ...
      <div className="space-y-3">
        ...
        {typeFields.command && action.type === 'command' ? (
          <div className="space-y-2">
            ...
            {typeFields.args && labels.executionArgsLabel ? (
              <div className="space-y-2">
                <Label className="text-sm">{labels.executionArgsLabel}</Label>
                <Input
                  ...
                  placeholder={labels.executionArgsPlaceholder}
                />
                {labels.executionArgsHelp ? <p className="text-xs text-muted-foreground">{labels.executionArgsHelp}</p> : null}
              </div>
            ) : null}
          </div>
        ) : null}
        ...
        {action.type === 'command' && providerSupport.additionalContextLimit && labels.executionAdditionalContextLimitLabel ? (
          <div className="space-y-2">
            <Label className="text-sm">{labels.executionAdditionalContextLimitLabel}</Label>
            <Input
              type="number"
              min={0}
              value={action.additionalContextLimit ?? ''}
              onChange={(event) => onChange({
                additionalContextLimit: event.target.value === '' ? null : Number(event.target.value),
              })}
              placeholder={labels.executionAdditionalContextLimitPlaceholder}
            />
            {labels.executionAdditionalContextLimitHelp ? <p className="text-xs text-muted-foreground">{labels.executionAdditionalContextLimitHelp}</p> : null}
          </div>
        ) : null}

        {action.type === 'command' && providerSupport.commandWindows && labels.executionCommandWindowsLabel ? (
          <div className="space-y-2">
            <Label className="text-sm">{labels.executionCommandWindowsLabel}</Label>
            <Input
              value={action.commandWindows ?? ''}
              onChange={(event) => onChange({ commandWindows: event.target.value || null })}
              placeholder={labels.executionCommandWindowsPlaceholder}
            />
            {labels.executionCommandWindowsHelp ? <p className="text-xs text-muted-foreground">{labels.executionCommandWindowsHelp}</p> : null}
          </div>
        ) : null}
