...
import enGemini from './en/modules/workspace/gemini';
import zhTWGemini from './zh-TW/modules/workspace/gemini';
...
describe('workspace settings labels', () => {
  it('keeps provider main navigation labels localized', () => {
    ...
    expect(enNavigation.main.geminiSettings).toBe('Gemini Settings');

    expect(zhTWNavigation.main.claudeCodeSettings).toBe('Claude Code 設定');
    expect(zhTWNavigation.main.codexSettings).toBe('Codex 設定');
    expect(zhTWNavigation.main.geminiSettings).toBe('Gemini 設定');
  });

  it('keeps Claude Code, Codex, and Gemini settings submenus in English', () => {
  ...
  it('keeps provider document and settings section titles in English for zh-TW', () => {
    ...
    expect(zhTWAgentSettings.codex.plugins.title).toBe('Extensions');
