...
- The Marketplace Hook model includes resource items, Marketplace i18n, and native package JSON projection, and is not promoted to a target-client-neutral Shared Hook contract; Detail tabs, Settings Version Control, and the Action Dialog also have no equivalent Shared DOM/state contract, so they are not forcibly replaced.
...
| Marketplace files | Second column/sidebar workflow/viewer workbench or read-only workbench | Marketplace target-client API, read-only or editor mutation | The controlled manager directly satisfies the constrained generic; dialog-state generic narrowing stays at the render-body boundary |
