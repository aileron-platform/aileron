...
建立兩個 namespace 的流程請見 [Kubernetes 快速安裝 — 準備 namespace 與
Secrets](./kubernetes.md#1-準備-namespace-與-secrets)。更新 CA 或 Registry 設定後，先在
每個 node 實際 pull digest；若使用 Secret-based 認證，再確認兩個 namespace 的 Secret，
最後執行 Helm upgrade。
