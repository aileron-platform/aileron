...
async function listenOnEphemeralPort(expressApp) {
...
test("Next.js dev server config starts from manifest contentDir", () => {
  ...
  assert.equal(config.options.uid, undefined);
  assert.equal(config.options.gid, undefined);
  assert.equal(config.options.env.HOME, "/tmp/aileron-canvas/home");
  assert.equal(config.options.env.npm_config_cache, "/tmp/aileron-canvas/npm-cache");
});

test("readiness endpoint returns 503 while the renderer is not ready", async () => {
