...
import type { GlowEffectManager } from "./GlowEffectManager";
...
export class CDPRouter {
  ...
  async handleCommand(msg: ExtensionCommandMessage): Promise<unknown> {
    ...
    switch (msg.params.method) {
      ...
      case "Browser.setGlowEffect": {
        const { enabled } = msg.params.params as { enabled: boolean };
        if (targetTabId) {
          await this.glowEffect.setGlow(targetTabId, enabled);
          return { success: true };
        }
        throw new Error("Tab not found for setGlowEffect");
      }

      case "Browser.getGlowState": {
        if (targetTabId) {
          const state = await this.glowEffect.getGlowState(targetTabId);
          return state;
        }
        throw new Error("Tab not found for getGlowState");
      }
    }
  ...
  handleDebuggerEvent(
    ...
    // Handle page load events - re-inject glow if needed
    if (method === "Page.loadEventFired" && source.tabId) {
      this.glowEffect.handlePageLoad(source.tabId);
    }
