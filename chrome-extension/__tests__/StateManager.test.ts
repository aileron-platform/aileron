...
describe("StateManager", () => {
  ...
  describe("getState", () => {
    it("should return default inactive state when no stored state", async () => {
      ...
      expect(state).toEqual({ isActive: false, glowEnabled: true });
    });
    ...
    it("should return stored state when available", async () => {
      ...
      const state = await stateManager.getState();
      expect(state).toEqual({ isActive: true, glowEnabled: true });
    });
  });

  describe("setState", () => {
    it("should persist state to storage", async () => {
      await stateManager.setState({ isActive: true, glowEnabled: true });
    ...
    it("should update state from active to inactive", async () => {
      await stateManager.setState({ isActive: true, glowEnabled: true });
      await stateManager.setState({ isActive: false, glowEnabled: true });
