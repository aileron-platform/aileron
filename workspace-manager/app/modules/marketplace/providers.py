...
class BaseMarketplaceProviderAdapter:
    ...
    def source_type_from_metadata(self, *items: dict[str, Any]) -> str:
        for item in items:
            ...
            if value in {"created", "imported", "cloned"}:
                return str(value)
        for item in items:
            if isinstance(item.get("importSource"), dict) or isinstance(
                item.get("import_source"), dict
            ):
                return "imported"
            if isinstance(item.get("clonedFrom"), dict) or isinstance(
                item.get("cloned_from"), dict
            ):
                return "cloned"
        return "created"
    ...
    def indexed_resource_names(self, package_path: Path) -> list[str]:
        names: set[str] = set()
        contract = provider_resource_name_contract(self.provider)
...
class MarketplaceManifestAdapter(BaseMarketplaceProviderAdapter):
    ...
    def _component_selector_target_identities(
        ...
        """Return provider logical target identities, independent of source paths."""
...
def create_marketplace_adapters() -> (
    dict[MarketplaceProvider, MarketplaceProviderAdapter]
):
