...
from app.modules.identity.oauth_models import (
    OAuthExchangeRequest,
    OAuthExchangeResponse,
    OAuthRefreshRequest,
    OAuthRefreshResponse,
)
from app.modules.identity.oauth import OAuthService
...
async def exchange_oauth_code(
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=_translate_oauth_error(
                request.state.translate, category="exchange_failed"
            ),
        )
...
async def authenticate_and_save(
    ...
    try:
        ...
        try:
            ...
            logger.info(
                f"Account: {account_info.email_address} ({account_info.display_name})"
            )
            logger.info(
                f"Organization: {account_info.organization_name} (Role: {account_info.organization_role})"
            )
        except Exception as e:
        ...
        claude_additional["subscriptionExpiresAt"] = (
            result.expires_at
        )  # Save millisecond timestamp directly
        ...
        logger.info(
            f"Successfully saved OAuth tokens and account info for user {user_id}"
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY,
            detail=_translate_oauth_error(
                http_request.state.translate, category="provider"
            ),
        )
    except ValueError as e:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=_translate_oauth_error(
                http_request.state.translate, category="authenticate_failed"
            ),
        )
...
async def refresh_oauth_token(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=_translate_oauth_error(
                http_request.state.translate, category="refresh_failed"
            ),
        )
