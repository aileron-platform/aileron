...
def source_service(mock_db_session, kb, tmp_path):
    ...
    service.settings.KB_ALLOWED_EXTENSIONS = [
        ".md",
        ".txt",
        ".csv",
        ".pdf",
...
@pytest.mark.unit
def test_import_file_writes_to_raw_sources_and_records_metadata(
    source_service, kb, tmp_path
):
    ...
    source_service.kb_service.get_kb_for_operation.assert_called_once_with(
        actor=OWNER_ACTOR,
        kb_id=kb.id,
    ...
    meta_path = (
        source_service.storage_root / kb.id / ".aileron-kb/sources-metadata.json"
    )
    ...
    activity = [
        call.args[0]
        for call in source_service.db.add.call_args_list
        if isinstance(call.args[0], db_models.PlatformResourceActivityEvent)
    ]
    assert [event.event_type for event in activity] == ["source_imported"]
...
def test_import_file_rejects_disallowed_extension(source_service, tmp_path):
    ...
    source_service.db.commit.assert_not_called()
...
def test_import_web_clip_writes_to_raw_sources_and_records_origin(source_service, kb):
    ...
    asset_file = (
        source_service.storage_root / kb.id / "raw/assets/example-page/diagram.png"
    )
...
def test_import_web_clip_checks_quota_before_writing(source_service, kb):
    ...
    assert not (
        source_service.storage_root / kb.id / "raw/sources/large-clip.md"
    ).exists()
