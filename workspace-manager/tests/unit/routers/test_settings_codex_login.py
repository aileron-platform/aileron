...
from app.routers.settings import get_codex_login_service
    ...
    with session_factory() as session:
        session.add(
            db_models.Workspace(
                ...
                runtime_external_url="http://localhost:65535",
            )
        )
        session.commit()
...
def test_codex_login_status_persists_backend_cli_state_but_hides_public_response(
    ...
    monkeypatch.setattr(
        "app.routers.settings._sync_settings_to_runtimes",
...
def test_codex_logout_clears_manager_source_state(
    ...
    assert synced_changes[0]["codex"]["loginStatus"] == "notConnected"
    assert synced_changes[0]["codex"].get("cliState") is None
    with session_factory() as session:
        settings = session.get(db_models.UserSetting, f"settings-{user.id}")
        assert settings is not None
        codex = settings.additional_settings["codex"]
