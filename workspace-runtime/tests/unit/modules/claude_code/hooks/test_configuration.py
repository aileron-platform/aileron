...
from app.modules.claude_code.hooks.configuration import HookService
...
class TestListScopes:
    ...
    @patch("app.modules.claude_code.hooks.configuration.read_json_file")
    @patch("app.modules.claude_code.hooks.configuration.resolve_scope_root")
    ...
    def test_list_scopes_plugin_only(
        self, mock_sources, mock_load_plugins, hook_service
    ):
...
class TestUpdateScope:
    ...
    @patch("app.modules.claude_code.hooks.configuration.write_json_file")
    @patch("app.modules.claude_code.hooks.configuration.read_json_file")
    @patch("app.modules.claude_code.hooks.configuration.resolve_scope_root")
    def test_update_scope_success(
        ...
        def read_persisted(_path):
            return deepcopy(persisted)

        def write_persisted(_path, data):
            persisted.clear()
            persisted.update(deepcopy(data))

        mock_read_json.side_effect = read_persisted
        mock_write_json.side_effect = write_persisted

        hook_action = HookAction(type=HookActionType.COMMAND, command="echo test")
        hook_rule = HookRule(matcher="user_prompt_submit", hooks=[hook_action])
        payload = HookScopeUpsertRequest(
            hooks={"test-hook": [hook_rule]},
        ...
        # Assert
        assert result is not None
        assert result.revision != _empty_revision()
        assert mock_write_json.called

    def test_update_scope_plugin_read_only(self, hook_service):
        payload = HookScopeUpsertRequest(hooks={}, revision=_empty_revision())
    ...
    @patch("app.modules.claude_code.hooks.configuration.read_json_file")
    @patch("app.modules.claude_code.hooks.configuration.resolve_scope_root")
    def test_update_scope_rejects_stale_revision(
        self, mock_resolve, mock_read_json, mock_write_json, hook_service, tmp_path
    ):
        mock_resolve.return_value = tmp_path
        mock_read_json.return_value = {
            "hooks": {
                "PreToolUse": [
                    {
                        "matcher": "*",
                        "hooks": [{"type": "command", "command": "echo current"}],
                    }
                ]
            }
        }
        ...
        with pytest.raises(HTTPException) as exc:
            hook_service.update_scope(
                "test-workspace",
                DocumentScope.PROJECT,
                HookScopeUpsertRequest(hooks={}, revision=_empty_revision()),
            )

        assert exc.value.status_code == 409
        assert exc.value.detail["errorCode"] == "REVISION_CONFLICT"
        assert not mock_write_json.called

    def test_update_scope_requires_revision(self):
        with pytest.raises(ValidationError):
            HookScopeUpsertRequest(hooks={})
...
class TestDeleteScope:
    ...
    @patch("app.modules.claude_code.hooks.configuration.resolve_scope_root")
    def test_delete_scope_success(self, mock_resolve, hook_service, tmp_path):
...
class TestExportScopes:
    ...
    @patch("app.modules.claude_code.hooks.configuration.read_json_file")
    @patch("app.modules.claude_code.hooks.configuration.resolve_scope_root")
    def test_export_scopes_success(
        self, mock_resolve, mock_read_json, hook_service, tmp_path
    ):
...
class TestImportScopes:
    ...
    def test_import_scopes_success(
            ...
            hooks={
                "test-hook": [
                    HookRule(
                        matcher="user_prompt_submit",
                        hooks=[
                            HookAction(type=HookActionType.COMMAND, command="echo test")
                        ],
                    )
                ]
            },
        ...
        import_data = HookImportRequest(mode=HookImportMode.MERGE, scopes=[scope_doc])
    ...
    def test_import_scopes_merge_skip_and_replace_counts(
        ...
        existing = HookScopeDocument(
            scope=DocumentScope.PROJECT, hooks={"event": [rule]}
        )
    ...
    def test_import_scopes_replace_updates_existing(
        ...
        request = HookImportRequest(
            ...
            scopes=[
                HookScopeDocument(scope=DocumentScope.PROJECT, hooks={"new": [rule]})
            ],
        )
...
class TestHookServiceInternals:
    @patch("app.modules.claude_code.hooks.configuration.resolve_scope_root")
    def test_scope_file_uses_official_settings_files_for_writable_claude_scopes(
        self, mock_resolve, hook_service, tmp_path, monkeypatch
    ):
        mock_resolve.return_value = tmp_path
        monkeypatch.setenv("HOME", str(tmp_path))

        assert (
            hook_service._scope_file("test-workspace", DocumentScope.PROJECT)
            == tmp_path / "settings.json"
        )
        assert (
            hook_service._scope_file("test-workspace", DocumentScope.USER)
            == tmp_path / ".claude" / "settings.json"
        )
        assert (
            hook_service._scope_file("test-workspace", DocumentScope.LOCAL)
    ...
    def test_merge_hooks_and_build_plugin_sources(self, hook_service):
        ...
        merged = hook_service._merge_hooks(
            {"one": [current_rule]}, {"two": [plugin_rule]}
        )
    ...
    def test_load_plugin_hooks_merges_plugin_metadata(self, hook_service, monkeypatch):
                ...
                "PreToolUse": [
                    {
                        ...
                        "hooks": [
                            {
                                "type": "command",
                                "command": "${CLAUDE_PLUGIN_ROOT}/bin/hook",
                                "headers": {
        ...
        fake_settings_module = ModuleType(
            "app.modules.claude_code.settings.dependencies"
        )
        fake_settings_module.get_settings_service = lambda: MagicMock()
        monkeypatch.setitem(
            sys.modules, "app.modules.claude_code.plugins.loader", fake_loader_module
        )
        monkeypatch.setitem(
            sys.modules,
            "app.modules.claude_code.settings.dependencies",
        ...
        assert hooks["PreToolUse"][0].hooks[0].command == "${PLUGIN_ROOT}/bin/hook"
        assert hooks["PreToolUse"][0].hooks[0].headers == {
