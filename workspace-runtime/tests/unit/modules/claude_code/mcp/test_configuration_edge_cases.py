...
class TestPathResolution:
    ...
    @patch("app.modules.claude_code.mcp.configuration.workspace_root")
    def test_project_file_path(
        self, mock_workspace_root, mcp_service, workspace_id, tmp_path
    ):
    ...
    @patch("app.modules.claude_code.mcp.configuration.workspace_root")
    def test_possible_user_project_keys(
        self, mock_workspace_root, mcp_service, workspace_id, tmp_path
    ):
    ...
    @patch("app.modules.claude_code.mcp.configuration.workspace_root")
    def test_primary_user_project_key(
        self, mock_workspace_root, mcp_service, workspace_id, tmp_path
    ):
...
class TestImportServers:
    ...
    def test_import_servers_skip_existing(
        ...
        payload = McpImportRequest(
            ...
            revision=_revision_for_servers(
                {"existing-server": {"type": "stdio", "command": "old-command"}}
            ),
    ...
    def test_import_servers_empty_payload(
        self, mock_read, mcp_service, workspace_id, tmp_path, sample_server_config
    ):
        ...
        payload = McpImportRequest(
            ...
            mcpServers={
                "": sample_server_config
            },  # Will be filtered out by _prepare_payload
            overwrite=False,
        )
...
class TestImportServersFromFile:
    ...
    def test_import_from_file_success(
        self, mock_read, mock_write, mcp_service, workspace_id, tmp_path
    ):
    ...
    def test_import_from_file_missing_mcp_servers_field(
        self, mcp_service, workspace_id
    ):
...
class TestLoadDisabledServers:
    ...
    def test_load_disabled_servers_success(self, mock_read, mcp_service, workspace_id):
        ...
        # Arrange
        mock_read.return_value = {
            "projects": {"/workspace": {"disabledMcpServers": ["server1", "server2"]}}
        }
    ...
    def test_load_disabled_servers_empty(self, mock_read, mcp_service, workspace_id):
        ...
        # Arrange
        mock_read.return_value = {"projects": {"/workspace": {}}}
    ...
    @patch("app.modules.claude_code.mcp.configuration.read_json_file")
    def test_load_disabled_servers_not_a_dict(
        self, mock_read, mcp_service, workspace_id
    ):
    ...
    @patch("app.modules.claude_code.mcp.configuration.read_json_file")
    def test_load_disabled_servers_projects_not_a_dict(
        self, mock_read, mcp_service, workspace_id
    ):
...
class TestUpdateProjectBlockField:
    ...
    @patch("app.modules.claude_code.mcp.configuration.write_json_file")
    @patch("app.modules.claude_code.mcp.configuration.read_json_file")
    def test_update_project_block_field_creates_new(
        self, mock_read, mock_write, mcp_service, workspace_id
    ):
    ...
    @patch("app.modules.claude_code.mcp.configuration.write_json_file")
    @patch("app.modules.claude_code.mcp.configuration.read_json_file")
    def test_update_project_block_field_preserves_other_fields(
        self, mock_read, mock_write, mcp_service, workspace_id
    ):
...
class TestRuntimeMap:
    ...
    @patch("app.modules.claude_code.mcp.configuration.read_json_file")
    def test_runtime_map_with_disabled_servers(
        self, mock_read, mcp_service, workspace_id, sample_server_entry
    ):
        """Test runtime map with disabled servers"""
        # Arrange
        mock_read.return_value = {
            "projects": {"/workspace": {"disabledMcpServers": ["server1"]}}
        }
        ...
        # Act
        result = mcp_service._runtime_map(
            entries,
            workspace_id,
        )
...
class TestMcpServiceAdditionalCoverage:
    ...
    @patch("app.modules.claude_code.mcp.configuration.read_json_file")
    def test_load_local_entries_uses_first_matching_project_key(
        self, mock_read, mcp_service
    ):
        mock_read.return_value = {
            "projects": {
                "/workspace": {
    ...
    @patch("app.modules.claude_code.mcp.configuration.read_json_file")
    def test_load_local_entries_returns_empty_for_non_dict_projects(
        self, mock_read, mcp_service
    ):
    ...
    @patch("app.modules.claude_code.mcp.configuration.write_json_file")
    @patch("app.modules.claude_code.mcp.configuration.read_json_file")
    def test_write_user_entries_preserves_other_root_fields(
        self, mock_read, mock_write, mcp_service, sample_server_entry
    ):
    ...
    @patch("app.modules.claude_code.mcp.configuration.write_json_file")
    @patch("app.modules.claude_code.mcp.configuration.read_json_file")
    def test_write_local_entries_updates_only_mcp_servers(
        self, mock_read, mock_write, mcp_service, sample_server_entry
    ):
        mock_read.return_value = {
    ...
    @patch.object(McpService, "_load_plugin_mcp_servers")
    def test_list_servers_logs_and_skips_plugin_load_failures(
        self, mock_plugin, mock_entries, mcp_service, caplog
    ):
    ...
    def test_update_server_rejects_empty_payload(
        self, mcp_service, sample_server_entry
    ):
        with patch.object(
            mcp_service,
            "_load_scope_entries",
            return_value={"srv": sample_server_entry},
        ):
            payload = McpServerUpdateRequest(
                revision=_revision_for_servers(
                    {"srv": sample_server_entry.to_storage()}
                ),
                mcpServers={"": sample_server_entry.config},
            )
            with pytest.raises(ValueError, match="Empty payload"):
                mcp_service.update_server("ws-1", DocumentScope.PROJECT, "srv", payload)

    def test_update_server_rejects_multiple_payload_items(
        self, mcp_service, sample_server_config
    ):
        entries = {"srv": McpServerEntry("srv", sample_server_config)}
        payload = McpServerUpdateRequest(
            revision=_revision_for_servers({"srv": entries["srv"].to_storage()}),
    ...
    def test_update_server_rejects_name_mismatch(
        self, mcp_service, sample_server_config
    ):
        entries = {"srv": McpServerEntry("srv", sample_server_config)}
        payload = McpServerUpdateRequest(
            revision=_revision_for_servers({"srv": entries["srv"].to_storage()}),
            mcpServers={"other": sample_server_config},
        )

        with patch.object(mcp_service, "_load_scope_entries", return_value=entries):
            with pytest.raises(
                McpServerAlreadyExistsError, match="does not match target"
            ):
                mcp_service.update_server("ws-1", DocumentScope.PROJECT, "srv", payload)
    ...
    def test_toggle_server_status_missing_server_raises(self, mcp_service):
        with patch.object(mcp_service, "_load_scope_entries", return_value={}):
            with pytest.raises(McpServerNotFoundError):
                mcp_service.toggle_server_status(
                    "ws-1", DocumentScope.PROJECT, "missing", False, EMPTY_REVISION
                )
    ...
    def test_toggle_server_status_updates_disabled_server_list(
        ...
        disabled = mcp_service.toggle_server_status(
            "ws-1",
            DocumentScope.PROJECT,
            "srv",
            False,
            _revision_for_servers({"srv": sample_server_entry.to_storage()}),
        )
        enabled = mcp_service.toggle_server_status(
            "ws-1",
            DocumentScope.PROJECT,
            "srv",
            True,
            _revision_for_servers({"srv": sample_server_entry.to_storage()}, ["srv"]),
        )
    ...
    def test_import_from_file_invalid_server_config_object(
        self, mcp_service, workspace_id
    ):
    ...
    def test_import_from_file_wraps_unexpected_exception(
        self, mcp_service, workspace_id
    ):
        file_content = json.dumps(
        ...
        with patch.object(
            mcp_service, "import_servers", side_effect=RuntimeError("boom")
        ):
    ...
    def test_load_plugin_mcp_servers_filters_invalid_entries_and_disabled_states(
        ...
        assert servers["chrome-devtools"].command == "${PLUGIN_ROOT}/bin/server"
    ...
    def test_toggle_plugin_server_status_enable_and_disable(
        ...
        disabled_servers = make_runtime()
        enabled_servers = make_runtime()
        mock_load_plugins.side_effect = [disabled_servers, enabled_servers]
        mock_load_disabled.side_effect = [
            ["chrome-devtools", "plugin:chrome-devtools-mcp:chrome-devtools"],
            [],
        ]

        disabled = mcp_service._toggle_plugin_server_status(
            "ws-1",
            "chrome-devtools",
            False,
            mcp_service._runtime_revision(disabled_servers),
        )
        enabled = mcp_service._toggle_plugin_server_status(
            "ws-1",
            "chrome-devtools",
            True,
            mcp_service._runtime_revision(enabled_servers),
        )

        assert disabled.mcpServers["chrome-devtools"].enabled is False
        assert enabled.mcpServers["chrome-devtools"].enabled is True
        assert mock_save_disabled.call_args_list[0].args[1] == [
            "plugin:chrome-devtools-mcp:chrome-devtools"
        ]
        assert mock_save_disabled.call_args_list[1].args[1] == []

    @patch.object(McpService, "_load_plugin_mcp_servers")
    def test_toggle_plugin_server_status_requires_plugin_name(
        self, mock_load_plugins, mcp_service
    ):
        runtime = McpServerEntry(
            name="plain",
            config=McpServerConfig(type=McpTransportType.STDIO, command="cmd"),
        ...
        with pytest.raises(ValueError, match="is not a plugin server"):
            mcp_service._toggle_plugin_server_status(
                "ws-1",
                "plain",
                False,
                mcp_service._runtime_revision({"plain": runtime}),
            )
