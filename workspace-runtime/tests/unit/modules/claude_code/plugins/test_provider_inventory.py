...
from app.modules.claude_code.plugins import provider_inventory as inventory_module
...
def test_plugin_list_success_is_shared_until_manual_refresh(
    ...
    assert len(calls) == 1
    inventory_module.clear_claude_provider_inventory_cache(tmp_path)
    assert service._plugin_rows("workspace-1") == [{"id": "demo@registry"}]
    assert len(calls) == 2
...
def test_internal_inventory_keeps_provider_reported_symlink_root(
    ...
    snapshot = build_claude_provider_inventory_snapshot(
        [
            {
                "id": "demo@registry",
                "scope": "user",
