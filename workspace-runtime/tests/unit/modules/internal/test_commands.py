...
class TestSetupSSHKeys:
    ...
    async def test_setup_ssh_keys_success(self, internal_service, tmp_paths):
        ...
        assert result["authorized_keys_path"] == str(
            tmp_paths["ssh"] / "authorized_keys"
        )
    ...
    @pytest.mark.asyncio
    async def test_setup_ssh_keys_duplicate_public_key(
    ...
    @pytest.mark.asyncio
    async def test_setup_ssh_keys_preserves_existing_keys(
        self, internal_service, tmp_paths
    ):
    ...
    @pytest.mark.asyncio
    async def test_setup_ssh_keys_directory_permissions(
        self, internal_service, tmp_paths
    ):
...
class TestSetupClaudeCode:
    ...
    async def test_setup_claude_code_subscription(self, internal_service, tmp_paths):
        ...
        credentials = json.loads(
            (tmp_paths["claude"] / ".credentials.json").read_text()
        )
        ...
        assert (
            oct((tmp_paths["claude"] / ".credentials.json").stat().st_mode)[-3:]
            == "600"
        )
        ...
        assert (
            "test-access-token" not in (tmp_paths["home"] / ".claude.json").read_text()
        )
...
class TestSetupCodex:
    ...
    async def test_setup_codex_writes_env_block(self, internal_service, tmp_paths):
        ...
        request = CodexSettingsRequest(
            ...
            environment_variables=[
                EnvironmentVariable(
                    key="OPENAI_BASE_URL", value="https://example.test"
                ),
            ],
        )
    ...
    @pytest.mark.asyncio
    async def test_setup_codex_rejects_codex_home_override(
        self, internal_service, tmp_paths
    ):
    ...
    @pytest.mark.asyncio
    async def test_setup_codex_clears_auth_but_preserves_env(
        self, internal_service, tmp_paths
    ):
        ...
        (tmp_paths["codex"] / "config.toml").write_text('model = "gpt-5.6-sol"\n')
    ...
    @pytest.mark.asyncio
    async def test_setup_codex_writes_synchronized_cli_state(
        self, internal_service, tmp_paths
    ):
        ...
        assert (
            tmp_paths["codex"] / "config.toml"
        ).read_text() == '[projects."/workspace"]\ntrust_level = "trusted"\n'
        assert (
            tmp_paths["codex"] / "installation_id"
        ).read_text() == "installation-1\n"
        assert result["has_cli_auth"] is True
        assert result["has_config"] is True
        assert result["has_installation_id"] is True

    @pytest.mark.asyncio
    async def test_setup_codex_cli_state_replaces_auth_and_preserves_unprovided_config(
        self, internal_service, tmp_paths
    ):
        ...
        assert (
            tmp_paths["codex"] / "installation_id"
        ).read_text() == "new-installation\n"
        assert result["has_cli_auth"] is True
        assert result["has_config"] is True
        assert result["has_installation_id"] is True

    @pytest.mark.asyncio
    async def test_setup_codex_cli_state_merges_existing_config(
        self, internal_service, tmp_paths
    ):
    ...
    @pytest.mark.asyncio
    async def test_setup_codex_cli_state_does_not_require_codex_binary(
        self, internal_service, tmp_paths
    ):
        ...
        assert result["has_cli_auth"] is True
        assert (
            json.loads((tmp_paths["codex"] / "auth.json").read_text())["tokens"][
                "refresh_token"
            ]
            == "refresh-token"
        )

    def test_check_codex_status_reports_success_for_auth(
        self, internal_service, tmp_paths
    ):
    ...
    @pytest.mark.asyncio
    async def test_setup_claude_code_api_key_clears_stale_oauth_state(
        self, internal_service, tmp_paths
    ):
        ...
        (tmp_paths["claude"] / ".credentials.json").write_text(
            json.dumps(
                {"authMethod": "subscription", "claudeAiOauth": {"accessToken": "old"}}
            )
        )
        (tmp_paths["home"] / ".claude.json").write_text(
            json.dumps(
                {"oauthAccount": {"emailAddress": "old@example.com"}, "other": True}
            )
        )
    ...
    @pytest.mark.asyncio
    async def test_setup_claude_code_environment_variables(
        self, internal_service, tmp_paths
    ):
    ...
    @pytest.mark.asyncio
    async def test_setup_claude_code_subscription_no_env_vars(
        self, internal_service, tmp_paths
    ):
    ...
    @pytest.mark.asyncio
    async def test_setup_claude_code_auto_detect_auth_method(
        self, internal_service, tmp_paths
    ):
    ...
    @pytest.mark.asyncio
    async def test_setup_claude_code_normalize_expires_at_integer(
        self, internal_service, tmp_paths
    ):
    ...
    @pytest.mark.asyncio
    async def test_setup_claude_code_normalize_expires_at_iso_string(
        self, internal_service, tmp_paths
    ):
    ...
    @pytest.mark.asyncio
    @patch("app.modules.internal.commands.SettingsService")
    async def test_setup_claude_code_with_model_override(
        self, mock_settings_service, internal_service, tmp_paths
    ):
...
class TestSetupGitSettings:
    ...
    async def test_setup_git_settings_success(self, mock_run, internal_service):
        ...
        )  # set name, set email, verify name, verify email
...
class TestGetSetupStatus:
    ...
    async def test_get_setup_status(self, internal_service):
        ...
        # Act
        with patch.object(
            internal_service, "_check_ssh_status", return_value={"status": "success"}
        ):
            with patch.object(
                internal_service,
...
class TestCheckClaudeStatus:
    ...
    def test_check_claude_status_subscription(self, tmp_path):
        ...
        try:
            with patch(
                "app.modules.internal.commands.Path.home", return_value=tmp_path
            ):
                with patch(
                    "app.modules.internal.commands.get_settings"
                ) as mock_settings:
                    ...
                    ), f"Expected 'success' but got '{result['status']}'. Message: {result.get('message', 'N/A')}"
        finally:
...
class TestApplyFirewallSettings:
    ...
    @pytest.mark.asyncio
    async def test_apply_firewall_settings_renders_blocked_egress_rules(
                ...
                with patch.object(Path, "write_text") as mock_write_text:
                    with patch.object(Path, "chmod"):
                        with patch("subprocess.run") as mock_run:
                            ...
                            result = await internal_service.apply_firewall_settings(
                                request
                            )
        ...
        assert 'log "Egress mode: blocked' in rendered_script

    @pytest.mark.asyncio
    async def test_apply_firewall_settings_renders_allowlist_rules(
        self, internal_service
    ):
        template_content = self._load_firewall_template()
        ...
        assert "iptables -P OUTPUT DROP" in rendered_script
        assert (
            "DOMAIN_IPS=$(dig +short example.com A example.com AAAA" in rendered_script
        )
...
class TestSetupClaudeCodeEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_setup_claude_code_with_existing_malformed_claude_json(
        self, internal_service, tmp_paths
    ):
    ...
    @pytest.mark.asyncio
    async def test_setup_claude_code_with_empty_env_vars(
        self, internal_service, tmp_paths
    ):
    ...
    @pytest.mark.asyncio
    async def test_setup_claude_code_env_vars_with_special_chars(
        self, internal_service, tmp_paths
    ):
        ...
        env_vars = [
            ...
            EnvironmentVariable(
                key="VAR_WITH_BACKSLASH", value="value with \\backslash"
            ),
        ]
    ...
    @pytest.mark.asyncio
    async def test_setup_claude_code_env_vars_update_existing_bashrc(
        self, internal_service, tmp_paths
    ):
...
"""
    ...
    @pytest.mark.asyncio
    async def test_setup_claude_code_without_auth_method(
        self, internal_service, tmp_paths
    ):
        ...
        with patch.dict(
            "os.environ", {internal_service._auth_method_env: "old_value"}, clear=False
        ):
            # Act
            await internal_service.setup_claude_code(request)
    ...
    @pytest.mark.asyncio
    async def test_setup_claude_code_general_exception(
        self, internal_service, tmp_paths
    ):
    ...
    async def test_setup_git_settings_general_exception(self, internal_service):
        ...
        with patch("subprocess.run", side_effect=RuntimeError("Unexpected error")):
            ...
            with pytest.raises(Exception) as exc_info:
                await internal_service.setup_git_settings(request)
            assert "Git setup failed" in str(
                exc_info.value
            ) or "Unexpected error" in str(exc_info.value)


class TestCheckSSHStatusEdgeCases:
    """Test edge cases for SSH status check"""

    def test_check_ssh_status_public_key_not_in_authorized_keys(
        self, internal_service, tmp_paths
    ):
    ...
    def test_check_ssh_status_read_error(self, internal_service, tmp_paths):
        ...
        with patch.object(
            Path, "read_text", side_effect=PermissionError("Cannot read")
        ):
            # Act
            result = internal_service._check_ssh_status()
    ...
    def test_check_ssh_status_missing_authorized_keys(
        self, internal_service, tmp_paths
    ):
...
class TestClearClaudeOauthState:
    ...
    def test_clear_claude_oauth_state_with_invalid_claude_json(
        self, internal_service, tmp_paths
    ):
    ...
    def test_clear_claude_oauth_state_with_non_dict_claude_json(
        self, internal_service, tmp_paths
    ):
    ...
    def test_clear_claude_oauth_state_without_oauth_account(
        self, internal_service, tmp_paths
    ):
...
class TestCheckClaudeStatusEdgeCases:
    ...
    def test_check_claude_status_invalid_credentials_json(
        self, internal_service, tmp_paths
    ):
    ...
    def test_check_claude_status_subscription_empty_file(
        self, internal_service, tmp_paths
    ):
    ...
    def test_check_claude_status_api_key_ignores_stale_oauth_credentials(
        self, internal_service, tmp_paths
    ):
        ...
        credentials_file = tmp_paths["claude"] / ".credentials.json"
        credentials_file.write_text(
            json.dumps({"claudeAiOauth": {"accessToken": "old-token"}})
        )
