...
from app.modules.client_browser_relay.relay import RelayService
...
def pairing_claims(
    pairing_session_id: str = "pairing-session-one",
    ...
    return BrowserPairingClaims(
        ...
        expires_at=2_000_000_045,
        jti=f"jti-{pairing_session_id}",
    )


@pytest.mark.asyncio
async def test_register_playwright_client_rejects_duplicate_ids(
...
@pytest.mark.asyncio
async def test_send_to_playwright_broadcasts_to_all_clients(
    relay_service: RelayService,
) -> None:
    ws1 = FakeWebSocket()
    ws2 = FakeWebSocket()
...
@pytest.mark.asyncio
async def test_send_to_playwright_handles_targeted_and_broadcast_errors(
    relay_service: RelayService,
) -> None:
    ok_ws = FakeWebSocket()
...
@pytest.mark.asyncio
async def test_send_to_extension_waits_for_response(
    relay_service: RelayService,
) -> None:
    ws = FakeWebSocket()
    relay_service.extension_ws = ws
    relay_service.extension_pairing = pairing_claims()
...
@pytest.mark.asyncio
async def test_route_cdp_command_handles_local_targets(
    relay_service: RelayService,
) -> None:
...
@pytest.mark.asyncio
async def test_send_attached_to_target_deduplicates_known_targets(
    relay_service: RelayService,
) -> None:
    ws1 = FakeWebSocket()
    ws2 = FakeWebSocket()
...
@pytest.mark.asyncio
async def test_handle_target_attached_resolves_waiting_future(
    relay_service: RelayService,
) -> None:
...
@pytest.mark.asyncio
async def test_get_or_create_named_page_returns_existing_mapping(
    relay_service: RelayService,
) -> None:
    target = ConnectedTarget(
        session_id="session-1",
    ...
    async def fake_send_to_extension(
        method: str, params: dict[str, object], timeout: float = 30.0
    ):
...
@pytest.mark.asyncio
async def test_unregister_extension_closes_all_playwright_clients(
    relay_service: RelayService,
) -> None:
    ...
    assert relay_service.extension_pairing is None
...
@pytest.mark.asyncio
async def test_unregister_playwright_client_and_extension_tolerate_failures(
    relay_service: RelayService,
) -> None:
...
@pytest.mark.asyncio
async def test_register_extension_replaces_second_socket_and_binds_current_claims(
    relay_service: RelayService,
) -> None:
    ...
    relay_service.extension_pairing = pairing_claims("pairing-session-old")
    ...
    new_pairing = pairing_claims("pairing-session-new")
    await relay_service.register_extension(new_ws, new_pairing)

    assert old_ws.close_calls == [(4001, "Extension Replaced")]
    assert relay_service.extension_ws is new_ws
    assert relay_service.extension_pairing is new_pairing
    assert relay_service.extension_pairing.runtime_instance_id == "runtime-instance-one"
    assert relay_service.extension_pairing.browser_workload_identity == (
        "browser-workload-one"
    )
    assert relay_service.extension_pairing.pairing_session_id == "pairing-session-new"
    assert relay_service.extension_pairing.jti == "jti-pairing-session-new"
    assert relay_service.connected_targets == {}
    assert relay_service.named_pages == {}
    assert future.done() is True
...
@pytest.mark.asyncio
async def test_register_extension_tolerates_close_failure(
    relay_service: RelayService,
) -> None:
    relay_service.extension_ws = FailingWebSocket()  # type: ignore[assignment]
    relay_service.extension_pairing = pairing_claims("pairing-session-old")

    pairing = pairing_claims()
    await relay_service.register_extension(  # type: ignore[arg-type]
        FakeWebSocket(),
        pairing,
    )

    assert relay_service.extension_ws is not None
    assert relay_service.extension_pairing is pairing


@pytest.mark.asyncio
async def test_send_to_extension_timeout_and_error_response(
    relay_service: RelayService,
) -> None:
    relay_service.extension_ws = FakeWebSocket()  # type: ignore[assignment]
    relay_service.extension_pairing = pairing_claims()
...
@pytest.mark.asyncio
async def test_send_to_extension_requires_extension_connection(
    relay_service: RelayService,
) -> None:
    ...
    assert relay_service.get_status("localhost", 3002)["extensionConnected"] is False


@pytest.mark.asyncio
async def test_route_cdp_command_additional_local_paths(
    relay_service: RelayService,
) -> None:
    ...
    assert await relay_service.route_cdp_command("Target.attachToBrowserTarget") == {
        "sessionId": "browser"
    }
    assert (
        await relay_service.route_cdp_command(
            "Target.detachFromTarget", {"sessionId": "browser"}
        )
        == {}
    )
    assert (
        await relay_service.route_cdp_command(
            "Target.getTargetInfo", session_id="session-1"
        )
    ...
    with pytest.raises(Exception, match="not found"):
        await relay_service.route_cdp_command(
            "Target.attachToTarget", {"targetId": "missing"}
        )


@pytest.mark.asyncio
async def test_route_cdp_command_supports_fallback_paths(
    relay_service: RelayService,
) -> None:
    ...
    relay_service.connected_targets["session-1"] = ConnectedTarget(
        ...
        target_id="target-1",
        target_info=TargetInfo(
            targetId="target-1", title="Demo", url="https://example.com"
        ),
    )

    result = await relay_service.route_cdp_command("Target.getTargetInfo")
    detached = await relay_service.route_cdp_command(
        "Target.detachFromTarget", {"sessionId": "session-1"}
    )
    auto_attach = await relay_service.route_cdp_command(
        "Target.setAutoAttach", session_id="session-1"
    )
...
@pytest.mark.asyncio
async def test_target_detach_info_change_delete_named_page_and_create_timeout(
    relay_service: RelayService,
) -> None:
...
@pytest.mark.asyncio
async def test_target_detached_removes_matching_named_mapping(
    relay_service: RelayService,
) -> None:
    relay_service.send_to_playwright = AsyncMock()  # type: ignore[method-assign]
...
@pytest.mark.asyncio
async def test_named_pages_helpers_and_target_info_changed_unknown_target(
    relay_service: RelayService,
) -> None:
    relay_service.send_to_playwright = AsyncMock()  # type: ignore[method-assign]
    relay_service.named_pages["demo"] = "session-1"
...
@pytest.mark.asyncio
async def test_full_drain_closes_extension_and_all_cdp_clients(
    relay_service: RelayService,
) -> None:
    extension = FakeWebSocket()
    user_a = FakeWebSocket()
    user_b = FakeWebSocket()
    relay_service.extension_ws = extension  # type: ignore[assignment]
    relay_service.extension_pairing = pairing_claims()
    await relay_service.register_playwright_client("user-a", user_a)  # type: ignore[arg-type]
    await relay_service.register_playwright_client("user-b", user_b)  # type: ignore[arg-type]

    await relay_service.full_drain()

    assert relay_service.extension_ws is None
    assert relay_service.extension_pairing is None
    assert relay_service.playwright_clients == {}
    assert extension.close_calls == [(1012, "WORKSPACE_RUNTIME_DRAINING")]
    assert user_a.close_calls == [(1012, "WORKSPACE_RUNTIME_DRAINING")]
    assert user_b.close_calls == [(1012, "WORKSPACE_RUNTIME_DRAINING")]


@pytest.mark.asyncio
async def test_full_drain_cancels_request_registered_before_socket_send_finishes(
    relay_service: RelayService,
) -> None:
    extension = BlockingSendWebSocket()
    relay_service.extension_ws = extension  # type: ignore[assignment]
    relay_service.extension_pairing = pairing_claims()
    request = asyncio.create_task(relay_service.send_to_extension("forwardCDPCommand"))
    await extension.send_started.wait()

    await relay_service.full_drain()
    extension.release_send.set()

    with pytest.raises(RuntimeError, match="WORKSPACE_RUNTIME_DRAINING"):
        await request
    assert relay_service.extension_pending_requests == {}


@pytest.mark.asyncio
async def test_full_drain_reports_socket_close_failure_after_clearing_state(
    relay_service: RelayService,
) -> None:
    relay_service.extension_ws = FailingWebSocket()  # type: ignore[assignment]
    relay_service.extension_pairing = pairing_claims()
    healthy = FakeWebSocket()
    await relay_service.register_playwright_client("user-a", healthy)  # type: ignore[arg-type]

    with pytest.raises(RuntimeError, match="browser_relay_drain_incomplete"):
        await relay_service.full_drain()

    assert relay_service.extension_ws is None
    assert relay_service.extension_pairing is None
    assert relay_service.playwright_clients == {}
    assert healthy.close_calls == [(1012, "WORKSPACE_RUNTIME_DRAINING")]
