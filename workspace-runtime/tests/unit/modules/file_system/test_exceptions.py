...
def test_other_file_exceptions_use_expected_codes_and_messages() -> None:
    cases = [
        (FileAlreadyExistsException("/tmp/existing.txt"), FileErrorCode.FILE_ALREADY_EXISTS, 409),
        ...
        (InvalidPathException("../etc/passwd", "path traversal"), FileErrorCode.INVALID_PATH, 400),
        (PermissionDeniedException("/tmp/file", "write"), FileErrorCode.PERMISSION_DENIED, 403),
        (FileTooLargeException("/tmp/big.bin", 1024, 512), FileErrorCode.FILE_TOO_LARGE, 413),
        (
            ContentConflictException("/tmp/conflict.txt", "v1", "v2"),
            FileErrorCode.CONTENT_CONFLICT,
            409,
        ),
        (DirectoryNotEmptyException("/tmp/dir"), FileErrorCode.DIRECTORY_NOT_EMPTY, 400),
    ]
