...
from app.modules.canvas.publishing import CanvasService
...
def canvas_service() -> CanvasService:
    return CanvasService(
        ...
        canvas_api_url="http://localhost:3013",
        canvas_internal_url="http://localhost:3003",
    )
...
class TestDetectionAndRoutes:
    def test_detect_remote_manifest(self, canvas_service: CanvasService) -> None:
        with patch("app.modules.canvas.publishing.httpx.Client") as mock_client_class:
    ...
    def test_routes_falls_back_to_local_default(
        self, canvas_service: CanvasService
    ) -> None:
        with patch(
            "app.modules.canvas.publishing.httpx.Client",
...
class TestActionsAndHealth:
    def test_sync_posts_to_canvas_management(
        self, canvas_service: CanvasService
    ) -> None:
        with patch("app.modules.canvas.publishing.httpx.Client") as mock_client_class:
            mock_client = MagicMock()
    ...
    def test_sync_allows_missing_optional_metadata(
        self, canvas_service: CanvasService
    ) -> None:
        with patch("app.modules.canvas.publishing.httpx.Client") as mock_client_class:
            mock_client = MagicMock()
    ...
    def test_reset_reports_management_unavailable(
        self, canvas_service: CanvasService
    ) -> None:
        with patch(
            "app.modules.canvas.publishing.httpx.Client",
    ...
    def test_delete_manifest_is_idempotent_and_syncs(
        self, canvas_service: CanvasService, tmp_path: Path
    ) -> None:
    ...
    def test_review_note_lifecycle_is_workspace_scoped(
        ...
        target = CanvasReviewAreaTarget(
            rect=CanvasReviewRect(
                x=1, y=2, width=100, height=80, coordinateSpace="viewport"
            )
        )
