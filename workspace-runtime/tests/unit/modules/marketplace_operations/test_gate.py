...
def test_generation_persists_without_installation_state(tmp_path: Path) -> None:
    ...
    gate = MarketplaceTargetClientGate(MarketplaceMutationStore(root))

    assert gate.generation("codex") == 0
    assert gate.advance_generation("codex") == 1
    assert gate.advance_generation("claude-code") == 1

    restarted = MarketplaceTargetClientGate(MarketplaceMutationStore(root))
    assert restarted.generation("codex") == 1
    assert restarted.generation("claude-code") == 1
    state = (root / "target-client-cache-generation.json").read_text(encoding="utf-8")
...
def test_concurrent_generation_advances_are_not_lost(
    ...
    gate = MarketplaceTargetClientGate(MarketplaceMutationStore(tmp_path / "marketplace"))
    ...
    target_clients = ["codex" if index % 2 == 0 else "claude-code" for index in range(24)]

    with ThreadPoolExecutor(max_workers=8) as executor:
        list(executor.map(gate.advance_generation, target_clients))

    assert gate.generation("codex") == 12
    assert gate.generation("claude-code") == 12


def test_settings_mutation_holds_target_client_lock_and_advances_once(
    tmp_path: Path,
) -> None:
    gate = MarketplaceTargetClientGate(MarketplaceMutationStore(tmp_path / "marketplace"))
...
def test_invalid_target_client_is_rejected(tmp_path: Path) -> None:
    gate = MarketplaceTargetClientGate(MarketplaceMutationStore(tmp_path / "marketplace"))

    with pytest.raises(MarketplaceOperationError) as exc_info:
        gate.generation("unknown")

    assert exc_info.value.code == "marketplace.install.target_client_invalid"
