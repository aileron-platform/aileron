...
Test Codex and OpenCode MCP CRUD, import/export, toggle functionality.
"""
...
def _revision_for(servers: Dict[str, Any]) -> str:
    return compute_revision(json.dumps(servers, sort_keys=True, separators=(",", ":")))


def _scope_revision(
    svc: CliMcpService,
    workspace_id: str = "ws1",
    scope: CliMcpScope = CliMcpScope.PROJECT,
) -> str:
    return svc.get_scope(workspace_id, scope).revision
...
def workspace_path(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr(
        "app.modules.cli_settings.mcp.configuration.get_workspace_path",
        lambda: str(ws_path),
    )
    return ws_path
...
class TestJsonMcp:
    """JSON MCP tests with toggle disabled"""

    def _make_service(self, tmp_path: Path) -> CliMcpService:
        config = _make_json_config(
            tmp_path, McpTool.OPENCODE, "mcp", supports_toggle=False
        )
    ...
    def test_list_empty(self, tmp_path: Path, workspace_path: Path):
        ...
        assert all(s.revision == _empty_revision() for s in result.scopes)
    ...
    def test_create_and_list(self, tmp_path: Path, workspace_path: Path):
        ...
        assert result.revision == _revision_for(
            {
                "my-server": {
                    "type": "local",
        ...
        assert listed.scopes[0].revision == result.revision

    def test_create_rejects_stale_revision(self, tmp_path: Path, workspace_path: Path):
        svc = self._make_service(tmp_path)
        svc.create_servers(
            "ws1",
            CliMcpScope.PROJECT,
        ...
        with pytest.raises(HTTPException) as exc_info:
            svc.create_servers(
                ...
                CliMcpServerCreateRequest(
                    revision=_empty_revision(),
                    mcpServers={"other": CliMcpServerConfig(command="cmd")},
                ),
            )
    ...
    def test_update_server(self, tmp_path: Path, workspace_path: Path):
        ...
        svc.create_servers(
            ...
            CliMcpServerCreateRequest(
                revision=_empty_revision(),
                mcpServers={"srv": CliMcpServerConfig(command="old-cmd")},
            ),
        )
        revision = _scope_revision(svc)
        result = svc.update_server(
            ...
            CliMcpServerUpdateRequest(
                revision=revision,
                mcpServers={"srv": CliMcpServerConfig(command="new-cmd")},
            ),
        )
    ...
    def test_delete_server(self, tmp_path: Path, workspace_path: Path):
        ...
        svc.delete_server("ws1", CliMcpScope.PROJECT, "srv", _scope_revision(svc))
    ...
    def test_delete_not_found(self, tmp_path: Path, workspace_path: Path):
        ...
        with pytest.raises(CliMcpServerNotFoundError):
            svc.delete_server(
                "ws1", CliMcpScope.PROJECT, "nonexistent", _empty_revision()
            )
    ...
    def test_create_duplicate(self, tmp_path: Path, workspace_path: Path):
        ...
        payload.revision = _scope_revision(svc)
    ...
    def test_toggle_not_supported(self, tmp_path: Path, workspace_path: Path):
        ...
        with pytest.raises(CliMcpToggleNotSupportedError):
            svc.toggle_server_status(
                "ws1", CliMcpScope.PROJECT, "srv", False, _scope_revision(svc)
            )

    def test_enabled_always_true(self, tmp_path: Path, workspace_path: Path):
        """Tools without toggle support always report enabled"""
        svc = self._make_service(tmp_path)
        svc.create_servers(
            "ws1",
            CliMcpScope.PROJECT,
...
class TestCodexMcp:
    ...
    def test_toggle(self, tmp_path: Path, workspace_path: Path):
        ...
        # Disable
        result = svc.toggle_server_status(
            "ws1", CliMcpScope.PROJECT, "srv", False, _scope_revision(svc)
        )
        ...
        # Enable
        result = svc.toggle_server_status(
            "ws1", CliMcpScope.PROJECT, "srv", True, result.revision
        )
        assert result.mcpServers["srv"].enabled is True
    ...
    def test_plugin_scope_reads_manifest_relative_mcp_servers(
        self, tmp_path: Path, workspace_path: Path, monkeypatch
    ):
        ...
        monkeypatch.setattr(
            "app.modules.cli_settings.mcp.configuration.get_codex_path_resolver",
            lambda: codex_resolver,
        )
        package_root = (
            codex_resolver.codex_home / "plugins" / "cache" / "local" / "demo" / "1.2.3"
        )
        manifest = package_root / ".codex-plugin" / "plugin.json"
        manifest.parent.mkdir(parents=True, exist_ok=True)
        manifest.write_text(
            '{"name":"demo","version":"1.2.3","mcpServers":"./.mcp.json"}',
            encoding="utf-8",
        )
        ...
        svc = self._make_service(tmp_path)
        svc._codex_plugin_resolver._plugin_rows = (  # type: ignore[method-assign]
            lambda: svc._codex_plugin_resolver._provider_rows(
                {
                    "available": [],
                    "installed": [
                        {
        ...
        assert "demo@local:docs" in result.scopes[0].mcpServers
        server = result.scopes[0].mcpServers["demo@local:docs"]
        assert server.command == "npx"
        assert server.plugin_id == "demo@local"
        assert server.plugin_name == "demo"
        assert server.marketplace_name == "local"

    def test_plugin_scope_keeps_disabled_manifest_mcp_servers_read_only(
        self, tmp_path: Path, workspace_path: Path, monkeypatch
    ):
        codex_resolver = CodexPathResolver(
            user_home=tmp_path / "home" / "developer",
        ...
        manifest.parent.mkdir(parents=True, exist_ok=True)
        manifest.write_text(
            '{"name":"demo","version":"1.2.3","mcpServers":{"docs":{"command":"npx"}}}',
            encoding="utf-8",
        )
        config_path = codex_resolver.resolve(CodexLayer.USER, CodexResource.CONFIG)
        config_path.parent.mkdir(parents=True, exist_ok=True)
        ...
        assert server.effective is False

    def test_plugin_scope_mutations_are_rejected(
        ...
        with pytest.raises(ValueError, match="read-only"):
            svc.delete_server("ws1", CliMcpScope.PLUGIN, "srv", _empty_revision())
        with pytest.raises(ValueError, match="read-only"):
            svc.toggle_server_status(
                "ws1", CliMcpScope.PLUGIN, "srv", False, _empty_revision()
            )
        with pytest.raises(ValueError, match="read-only"):
            svc.import_servers(
                "ws1",
...
class TestOpenCodeMcp:
    ...
    def test_import_overwrite(self, tmp_path: Path, workspace_path: Path):
        ...
        import_req.revision = result.revision
        result = svc.import_servers("ws1", import_req)
...
class TestCrossToolCrud:
    ...
    def test_create_get_delete(self, tool_service: tuple[str, CliMcpService]):
        ...
        # Delete
        svc.delete_server("ws1", CliMcpScope.PROJECT, "test-srv", _scope_revision(svc))
        with pytest.raises(CliMcpServerNotFoundError):
