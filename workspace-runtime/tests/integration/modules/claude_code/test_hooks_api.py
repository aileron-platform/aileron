"""Claude Code API test cases - Hooks"""
...
def test_hook_001_list_all_scopes(client):
    ...
    with override_dependency(get_hook_service, lambda: service):
        response = client.get(f"/api/v1/workspaces/{WORKSPACE_ID}/claude-code/hooks")
...
def test_hook_005_update_scope(client):
    ...
    with override_dependency(get_hook_service, lambda: service):
        response = client.put(
            f"/api/v1/workspaces/{WORKSPACE_ID}/claude-code/hooks/project",
            json={"hooks": {}, "revision": compute_revision("{}")},
        )
...
def test_hook_error_envelope(client):
    service = StubHookService(
        scope_error=HTTPException(
            status_code=400,
            detail={"error": "UNSUPPORTED_SCOPE", "message": "Unsupported scope"},
        )
    )
