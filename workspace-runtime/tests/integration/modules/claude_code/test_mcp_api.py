...
class StubMcpService:
    ...
    updated_payloads: list[tuple[str, DocumentScope, str, McpServerUpdateRequest]] = (
        field(default_factory=list)
    )
    deleted: list[tuple[str, DocumentScope, str, str]] = field(default_factory=list)
    toggled: list[tuple[str, DocumentScope, str, bool, str]] = field(
        default_factory=list
    )
    imports: list[tuple[str, bytes, bool, str]] = field(default_factory=list)
    ...
    def delete_server(
        ...
        self.deleted.append((workspace_id, scope, server_name, revision))
    ...
    def toggle_server_status(
        ...
        self.toggled.append((workspace_id, scope, server_name, enabled, revision))
    ...
    def import_servers_from_file(
        ...
        self.imports.append(
            (workspace_id, payload.file, payload.overwrite, payload.revision)
        )
        ...
        list_result=McpServerCollectionResponse(
            ...
            scopes=[
                McpScopeServers(
                    ...
                    revision=EMPTY_REVISION,
                    mcpServers={
                        "cli": {"type": "stdio", "command": "echo", "enabled": True}
                    },
                )
            ],
        )
...
def test_mcp_003_get_scope_success(client):
    service = StubMcpService(
        scope_result=_mcp_scope(
            DocumentScope.PROJECT, {"cli": {"type": "stdio", "command": "echo"}}
        )
    )
...
def test_mcp_004_scope_not_supported(client):
    ...
    assert response.json()["detail"]["errorCode"] == "SCOPE_NOT_SUPPORTED"


def test_mcp_005_get_server_success(client):
    service = StubMcpService(
        server_result=_mcp_scope(
            DocumentScope.PROJECT, {"cli": {"type": "stdio", "command": "echo"}}
        )
    )
...
def test_mcp_006_get_server_missing(client):
    ...
    assert response.status_code == 404
    assert response.json()["detail"]["errorCode"] == "SERVER_NOT_FOUND"


def test_mcp_007_create_server_success(client):
    service = StubMcpService(
        create_result=_mcp_scope(
            DocumentScope.PROJECT, {"new": {"type": "stdio", "command": "cmd"}}
        )
    )
...
def test_mcp_008_create_duplicate(client):
    service = StubMcpService(create_error=McpServerAlreadyExistsError("exists"))
    ...
    assert response.json()["detail"]["errorCode"] == "409_DUPLICATE_NAME"
...
def test_mcp_009_create_invalid_payload(client):
    ...
    payload = {
        "revision": EMPTY_REVISION,
        "mcpServers": {"bad": {"type": "stdio", "command": "echo"}},
    }
    ...
    assert response.json()["detail"]["errorCode"] == "400_INVALID_PAYLOAD"


def test_mcp_010_update_server_success(client):
    service = StubMcpService(
        update_result=_mcp_scope(
            DocumentScope.PROJECT, {"cli": {"type": "stdio", "command": "cmd"}}
        )
    )
...
def test_mcp_011_update_conflict(client):
    service = StubMcpService(update_error=McpServerAlreadyExistsError("conflict"))
...
def test_mcp_012_delete_success(client):
    ...
    with override_dependency(get_mcp_service, lambda: service):
        response = client.delete(
            f"/api/v1/workspaces/{WORKSPACE_ID}/claude-code/mcp-servers/project/cli",
            params={"revision": EMPTY_REVISION},
        )
...
def test_mcp_014_toggle_status(client):
    service = StubMcpService(
        toggle_result=_mcp_scope(
            DocumentScope.PROJECT,
            {"cli": {"type": "stdio", "command": "echo", "enabled": False}},
        )
    )
    ...
    with override_dependency(get_mcp_service, lambda: service):
        response = client.patch(
            ...
            params={"enabled": False, "revision": EMPTY_REVISION},
        )

    assert response.status_code == 200
    assert service.toggled[0][3] is False
    ...
    with override_dependency(get_mcp_service, lambda: service):
        with file_path.open("rb") as fp:
            response = client.post(
                ...
                data={"scope": "user", "overwrite": "true", "revision": EMPTY_REVISION},
            )
    ...
    with override_dependency(get_mcp_service, lambda: service):
        with file_path.open("rb") as fp:
            response = client.post(
                ...
                data={"scope": "user", "revision": EMPTY_REVISION},
            )
