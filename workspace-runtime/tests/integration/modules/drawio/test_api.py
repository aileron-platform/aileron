...
class DrawioFileServiceStub:
    ...
    def write_file(
        self, request, scope: str = None, expected_version_id: str = None
...
def test_drawio_viewer_generates_url(client):
    ...
    with (
        override_dependency(get_file_service_sync, lambda: service),
        override_dependency(get_i18n_service, lambda: _translation_service),
    ):
        response = client.get(
...
def test_drawio_availability_endpoint_returns_state(
    client, monkeypatch, available, reason
):
