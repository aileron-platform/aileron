...
from app.modules.version_control.git_operations import VersionControlError
...
class ChangesResult:
    """Cursor-paged changes result used by the HTTP adapter tests."""
    ...
    def __init__(
        ...
        def page(items: list, total: int) -> dict[str, Any]:
            return {
                "items": items,
                "total": total,
        ...
        self.staged = page(staged, staged_total)
        self.unstaged = page(unstaged, unstaged_total)
        self.untracked = page(untracked, untracked_total)
        self.conflicts = page(conflicts, len(conflicts))
...
class StubGitService:
    ...
    def list_branches(
        ...
        if not include_remote:
            branches = [b for b in branches if not b.get("isRemote", False)]
        return {
            "branches": [
                {
                    "name": branch["name"],
                    "displayName": branch.get("displayName", branch["name"]),
                    "kind": "remote" if branch.get("isRemote", False) else "local",
                    "isCurrent": branch.get(
                        "isCurrent", branch.get("isActive", False)
                    ),
                    "upstream": None,
                    "ahead": branch.get("ahead", 0),
                    "behind": branch.get("behind", 0),
                    "checkedOutTarget": None,
                    "capabilities": {
                        "switch": {"allowed": True},
                        "rename": {"allowed": not branch.get("isRemote", False)},
                        "delete": {"allowed": not branch.get("isRemote", False)},
                    },
                }
    ...
    def get_status(
        ...
        return {
            "isInitialized": self.is_repo,
            "currentBranch": self.current_branch if self.is_repo else None,
            "detachedHead": False,
            "headSha": None,
            "hasOrigin": False,
            "upstream": None,
            ...
            "behind": 0,
            "hasConflicts": False,
            "stagedTotal": len(self.staged_files),
            "unstagedTotal": len(self.unstaged_files),
            "untrackedTotal": len(self.untracked_files),
            "conflictTotal": 0,
            "operationStatus": None,
        }

    def create_branch(self, workspace_id: str, **kwargs) -> dict[str, Any]:
        name = kwargs["name"]
        self.branches.append({"name": name, "isCurrent": True, "isRemote": False})
        self.current_branch = name
        return {"commandId": "branch.createAndSwitch", "branch": name}

    def switch_branch(self, workspace_id: str, **kwargs) -> dict[str, Any]:
        self.current_branch = kwargs["name"]
        return {"commandId": "branch.switch", "branch": self.current_branch}
    ...
    def get_changes(
        ...
        def selected(items: list[str], name: str) -> list[str]:
            return items[offset : offset + limit] if group in {"all", name} else []
        ...
        return ChangesResult(
            staged=[
                ...
                for path in selected(self.staged_files, "staged")
            ],
            unstaged=[
                {
                    "name": path.split("/")[-1] if "/" in path else path,
                ...
                for path in selected(self.unstaged_files, "unstaged")
            ],
            untracked=[
                {
                    "name": path.split("/")[-1] if "/" in path else path,
                ...
                for path in selected(self.untracked_files, "untracked")
            ],
            conflicts=[],
    ...
    def stage(
        self, workspace_id: str, payload, context_id: Optional[str] = None
    ) -> dict[str, Any]:
    ...
    def commit(
        ...
        if not message:
            raise VersionControlError(
                ...
                status_code=400,
                error_code="VC_COMMIT_MESSAGE_REQUIRED",
            )
    ...
    def list_commits(
        ...
        return {
            ...
            "nextCursor": str(end) if end < len(self.commits) else None,
            "hasMore": end < len(self.commits),
    ...
    def get_remote_settings(
        ...
        return {
            ...
            "hasOrigin": self.remote_url is not None,
        }

    def set_remote_settings(
        self, workspace_id: str, payload, context_id: Optional[str] = None
    ) -> dict[str, Any]:
        self.remote_url = payload.remote_url
        return {"commandId": "remote.settings.update"}

    def get_lfs_patterns(
        self, workspace_id: str, context_id: Optional[str] = None
    ) -> dict[str, Any]:
        return {"patterns": self.lfs_patterns}
    ...
    def update_lfs_patterns(
        ...
        *,
        ...
        self.lfs_patterns = patterns or ["*.pdf", "*.xlsx"]
        return {
            "commandId": "lfs.patterns.update",
            "affectedTotal": len(self.lfs_patterns),
        }
    ...
    def convert_lfs_snapshot(
        ...
        *,
        ...
        return {
            ...
            "affectedTotal": len(paths),
        }

    def cancel_operation(
        self, workspace_id: str, *, context_id: Optional[str] = None
    ) -> dict[str, Any]:
        return {"commandId": "operation.cancel"}
...
def test_vc_001_uninitialized_repository(client):
    ...
    class NonRepoGitService(StubGitService):
        ...
        def get_status(
            ...
            return super().get_status(workspace_id, context_id)
    ...
    assert payload["isInitialized"] is False
    assert payload["currentBranch"] is None
...
def test_vc_002_initialized_with_changes(client):
    ...
    assert payload["stagedTotal"] == 1
    assert payload["unstagedTotal"] == 1
...
def test_vc_003_list_branches_with_remote(client):
    ...
    with override_dependency(get_git_service, lambda: service):
        response = client.get(
            "/api/v1/workspaces/test_ws/version-control/branches?includeRemote=true"
        )
    ...
    remote_branches = [b for b in branches if b["kind"] == "remote"]
...
def test_vc_004_branches_search_filter(client):
    ...
    with override_dependency(get_git_service, lambda: service):
        response = client.get(
            "/api/v1/workspaces/test_ws/version-control/branches?search=feature"
        )
...
def test_vc_005_checkout_existing_branch(client):
    ...
    with override_dependency(get_git_service, lambda: service):
        response = client.post(
            "/api/v1/workspaces/test_ws/version-control/branches/switch",
            json={"name": "develop"},
        )
...
def test_vc_006_create_and_checkout_new_branch(client):
    ...
    with override_dependency(get_git_service, lambda: service):
        response = client.post(
            "/api/v1/workspaces/test_ws/version-control/branches/create",
            json={"name": "feature/api", "startPoint": "HEAD"},
        )
...
def test_vc_007_get_all_changes(client):
    ...
    assert response.status_code == 200
    payload = response.json()
    assert len(payload["staged"]["items"]) == 2
    assert len(payload["unstaged"]["items"]) == 1
    assert len(payload["untracked"]["items"]) == 1
    assert payload["conflicts"]["items"] == []
    assert payload["untracked"]["total"] == 1
...
def test_vc_008_scope_filter_changes(client):
    ...
    with override_dependency(get_git_service, lambda: service):
        response = client.get(
            "/api/v1/workspaces/test_ws/version-control/changes?group=staged"
        )

    assert response.status_code == 200
    payload = response.json()
    assert len(payload["staged"]["items"]) == 1
    assert len(payload["unstaged"]["items"]) == 0
    assert len(payload["untracked"]["items"]) == 0
    assert payload["conflicts"]["items"] == []
...
def test_vc_015_paginate_commits(client):
    ...
    with override_dependency(get_git_service, lambda: service):
        response = client.get(
            "/api/v1/workspaces/test_ws/version-control/commits?cursor=2&limit=2"
        )
...
def test_vc_016_get_commit_details(client):
    ...
    with override_dependency(get_git_service, lambda: service):
        response = client.get(
            "/api/v1/workspaces/test_ws/version-control/commits/commit_123"
        )
...
def test_vc_017_commit_not_found(client):
    ...
    with override_dependency(get_git_service, lambda: service):
        response = client.get(
            "/api/v1/workspaces/test_ws/version-control/commits/nonexistent"
        )
...
def test_vc_018_list_commit_files(client):
    ...
    with override_dependency(get_git_service, lambda: service):
        response = client.get(
            "/api/v1/workspaces/test_ws/version-control/commits/commit_123/files"
        )
...
def test_vc_028_remote_lfs_and_cancel_contract(client):
    service = StubGitService(Path("/tmp/workspace"))

    with override_dependency(get_git_service, lambda: service):
        remote_update = client.put(
            "/api/v1/workspaces/test_ws/version-control/remote",
            json={"remoteUrl": "https://example.com/repository.git"},
        )
        remote = client.get("/api/v1/workspaces/test_ws/version-control/remote")
        lfs = client.get("/api/v1/workspaces/test_ws/version-control/lfs")
        lfs_update = client.post(
            "/api/v1/workspaces/test_ws/version-control/lfs",
            json={"patterns": ["assets/**"]},
        )
        preview = client.post(
            "/api/v1/workspaces/test_ws/version-control/lfs/preview",
            json={"patterns": ["assets/**"]},
        )
        convert = client.post(
            "/api/v1/workspaces/test_ws/version-control/lfs/convert",
            json={"paths": ["assets/a.bin", "assets/b.bin"]},
        )
        cancel = client.post(
            "/api/v1/workspaces/test_ws/version-control/operation/cancel"
        )

    assert remote_update.status_code == 200
    ...
    assert lfs.json() == {"patterns": ["*.pdf"]}
    assert lfs_update.json()["commandId"] == "lfs.patterns.update"
    assert preview.json() == {
        "matchedTotal": 2,
        "totalSize": 4096,
        "pathSample": ["assets/a.bin", "assets/b.bin"],
    }
    assert convert.json()["affectedTotal"] == 2
