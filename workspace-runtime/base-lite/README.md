...
`base-lite` 是 `workspace-runtime` 的精簡 base image，和 `codex-universal` 平行存在。
...
```bash
docker build -t ailerondocker/workspace-runtime-base-lite:custom ./workspace-runtime/base-lite
```
