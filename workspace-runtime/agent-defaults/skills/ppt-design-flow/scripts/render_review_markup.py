...
from canvas_protocol import skill_session_dir

try:
    from PIL import Image, ImageDraw, ImageFont
except ImportError as exc:  # pragma: no cover - runtime dependency message
    raise SystemExit(
        "Pillow is required to render review markup. Install it with: python3 -m pip install pillow"
    ) from exc
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--out",
        type=Path,
        help="Explicit output directory for marked images and note text files.",
    )
    parser.add_argument(
        "--workspace",
        type=Path,
        help="Workspace directory used to derive the internal .aileron output path when --out is omitted.",
    )
    parser.add_argument(
        "--session-id",
        help="Canvas session id used to derive the internal .aileron output path when --out is omitted.",
    )
    parser.add_argument(
        "--image-map",
        type=Path,
...
def resolve_output_dir(args: argparse.Namespace) -> Path:
    if args.out:
        return args.out
    if not args.workspace or not args.session_id:
        raise SystemExit("Provide --out or both --workspace and --session-id.")
    return skill_session_dir(args.workspace, args.session_id) / "review-markup"
...
def main() -> None:
    ...
    for page in pages:
        ...
        output_path = output_dir / f"{page_id}_marked.png"
        marked.convert("RGB").save(output_path)
        write_note_text(output_dir / f"{page_id}_notes.txt", page)
