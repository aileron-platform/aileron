...
class RuntimeDrainErrorResponse(BaseModel):
    """Stable code-only Runtime drain error."""
...
class ClaudeCodeRequest(BaseModel):
    ...
    oauth_account: Optional[OAuthAccountInfo] = Field(
        None, alias="oauthAccount", description="OAuth account information"
    )
    api_key: Optional[str] = Field(None, alias="apiKey", description="Claude API Key")
...
class FirewallConfigRequest(BaseModel):
    ...
    egress_mode: Literal["blocked", "allowlist", "unrestricted"] = Field(
        ...,
        alias="egressMode",
        description="External network egress mode",
    )
    allowed_domains: List[str] = Field(
        default_factory=list, alias="allowedDomains", description="Allowed domain list"
    )

    model_config = ConfigDict(populate_by_name=True, extra="forbid")

    @model_validator(mode="after")
    def validate_allowed_domains(self) -> "FirewallConfigRequest":
        if self.egress_mode == "allowlist" and not self.allowed_domains:
            raise ValueError("allowedDomains must not be empty in allowlist mode")
        if self.egress_mode != "allowlist" and self.allowed_domains:
            raise ValueError("allowedDomains must be empty outside allowlist mode")
        return self


class CapabilitiesSyncRequest(BaseModel):
    """Workspace capabilities snapshot request."""

    workspace_id: str = Field(..., description="Workspace ID")
    capabilities: dict[str, Any] = Field(..., description="Capabilities snapshot")
...
class _StrictMarketplaceModel(BaseModel):
    """Reject unknown fields on every Marketplace operation boundary."""
...
class MarketplacePluginCommandResult(_StrictMarketplaceModel):
    """Bounded terminal CLI result without durable installation state."""
    ...
    cli_message: Optional[str] = Field(
        default=None,
        alias="cliMessage",
        max_length=4096,
    )
...
class WorkspaceSetupStatusResponse(BaseModel):
    ...
    message: str = Field(..., description="Response message")
    checks: Dict[str, SetupCheckDetail] = Field(
        default_factory=dict, description="Status of each check item"
    )
