...
from app.modules.cli_settings.user_scope.models import UserScopeAgent
...
def format_file_size(size_bytes: float) -> str:
    ...
    """
    normalized_size = float(size_bytes)
    for unit in ["B", "KB", "MB", "GB"]:
        if normalized_size < BYTES_PER_KB:
            return f"{normalized_size:.1f}{unit}"
        normalized_size /= BYTES_PER_KB
...
def resolve_scope_root(workspace_id: str, scope: DocumentScope) -> Path:
    ...
    if scope == DocumentScope.USER:
        return (
            ...
            .resolve_root(UserScopeAgent.CLAUDE_CODE)
            .runtime_path
        )
...
class ScopedMarkdownRepository:
    ...
    def _directory(self, workspace_id: str, scope: DocumentScope) -> Path:
        root = (
            self._scope_root_resolver(workspace_id, scope)
            if self._scope_root_resolver
            else resolve_scope_root(workspace_id, scope)
        )
    ...
    def _load_record(
        ...
        content = _MARKDOWN_CODEC.read(file_path)
    ...
    def list_records(
        ...
        for file_path in _MARKDOWN_CODEC.files(directory):
    ...
    def _resolve_file_path(
        ...
        candidates = [
            path
            for path in _MARKDOWN_CODEC.files(directory)
...
def check_scope_writable(scope: DocumentScope) -> None:
    ...
    if scope == DocumentScope.PLUGIN:
        raise ValueError(
            "Plugin scope is read-only. Plugins can only be managed through the marketplace."
        )
