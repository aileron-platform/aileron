...
def _scope_error(error: McpScopeNotSupportedError) -> NoReturn:
    raise_resource_error("SCOPE_NOT_SUPPORTED", str(error), status.HTTP_404_NOT_FOUND)
...
def _duplicate_error(error: McpServerAlreadyExistsError) -> NoReturn:
    raise_resource_error("409_DUPLICATE_NAME", str(error), status.HTTP_409_CONFLICT)


def _invalid_payload(error: ValueError) -> NoReturn:
    message = str(error) or "Invalid payload"
    raise_resource_error("400_INVALID_PAYLOAD", message, status.HTTP_400_BAD_REQUEST)
...
async def import_servers(
    ...
    file: UploadFile = File(
        ..., description="Claude Desktop configuration file (JSON)"
    ),
    overwrite: bool = Form(
    ...
    try:
        ...
        payload = McpImportUploadRequest(
            scope=scope, revision=revision, file=file_content, overwrite=overwrite
        )
