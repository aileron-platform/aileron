...
class McpService:
    ...
    def list_servers(
                ...
                return McpServerCollectionResponse(
                    ...
                    scopes=[
                        McpScopeServers(
                            ...
                            revision=self._runtime_revision(plugin_servers),
                            mcpServers=plugin_servers,
                        )
                    ],
                )

            scopes: tuple[DocumentScope, ...] = (scope,)
        else:
        ...
        for scope_item in scopes:
            ...
            groups.append(
                McpScopeServers(
                    scope=scope_item,
                    revision=self._entries_revision(workspace_id, scope_item, entries),
                    mcpServers=self._runtime_map(
                        entries,
                        workspace_id,
                    ),
                )
    ...
    def get_scope(self, workspace_id: str, scope: DocumentScope) -> McpScopeResponse:
        ...
        return McpScopeResponse(
            workspaceId=workspace_id,
            scope=normalized,
            revision=self._entries_revision(workspace_id, normalized, entries),
            mcpServers=self._runtime_map(
                entries,
                workspace_id,
            ),
    ...
    def get_server(
        ...
        response = McpScopeResponse(
            ...
            revision=self._entries_revision(workspace_id, normalized, entries),
            mcpServers={
                server_name: self._runtime_entry(
                    entry,
                    enabled=enabled,
                )
            },
    ...
    def create_servers(
        ...
        file_path = self._scope_file(workspace_id, normalized)
        with _write_lock(file_path):
            entries = self._load_scope_entries(workspace_id, normalized)
            assert_revision(
                self._entries_revision(workspace_id, normalized, entries),
                payload.revision,
            )
    ...
    def delete_server(
        self, workspace_id: str, scope: DocumentScope, server_name: str, revision: str
    ) -> McpServerDeleteResponse:
        normalized = self._normalize_scope(scope)
        file_path = self._scope_file(workspace_id, normalized)
        with _write_lock(file_path):
    ...
    def toggle_server_status(
        ...
        # Handle plugin scope
        if scope == DocumentScope.PLUGIN:
            return self._toggle_plugin_server_status(
                workspace_id, server_name, enabled, revision
            )

        # Handle general scope (project/user/local)
        normalized = self._normalize_scope(scope)
        file_path = self._local_file()
        with _write_lock(file_path):
            entries = self._load_scope_entries(workspace_id, normalized)
            disabled_servers = self._load_disabled_servers(workspace_id)
            assert_revision(
                self._entries_revision_with_disabled(entries, disabled_servers),
                revision,
            )
            if server_name not in entries:
                raise McpServerNotFoundError(server_name)
        ...
        return McpScopeResponse(
            workspaceId=workspace_id,
            scope=normalized,
            revision=self._entries_revision_with_disabled(entries, disabled_servers),
            mcpServers={
                server_name: self._runtime_entry(
                    entry,
                    enabled=enabled,
    ...
    def _scope_file(self, workspace_id: str, scope: DocumentScope) -> Path:
        if scope == DocumentScope.PROJECT:
            return self._project_file(workspace_id)
    ...
    def _possible_user_project_keys(self, workspace_id: str) -> List[str]:
        ...
        raw_candidates: List[str | None] = [
            ...
            (
                f"/{workspace_id}" if not workspace_id.startswith("/") else workspace_id
            ),  # Relative to workspace_id
    ...
    def _load_project_entries(self, workspace_id: str) -> Dict[str, McpServerEntry]:
    ...
    def _load_local_entries(self, workspace_id: str) -> Dict[str, McpServerEntry]:
    ...
    def _load_user_entries(
        self,
        workspace_id: str,  # pylint: disable=unused-argument
    ) -> Dict[str, McpServerEntry]:
    ...
    def _write_user_entries(
        self,
        workspace_id: str,
        entries: Dict[str, McpServerEntry],  # pylint: disable=unused-argument
    ) -> None:
        """User scope uses ~/.claude.json direct structure: root -> mcpServers"""
        path = self._user_file()
        data = read_json_file(path)
    ...
    def _runtime_entry(
        ...
        *,
        ...
        return entry.to_runtime(enabled=enabled)
    ...
    def _entries_revision(
        self,
        workspace_id: str,
        scope: DocumentScope,
        entries: Dict[str, McpServerEntry],
    ) -> str:
        encoded = self._encode_servers(entries)
        disabled = [
            name
            for name in encoded
            if name in self._load_disabled_servers(workspace_id)
        ]
        content = {
            "mcpServers": encoded,
            "disabledMcpServers": sorted(disabled),
        }
        return compute_revision(
            json.dumps(content, sort_keys=True, separators=(",", ":"))
        )

    def _entries_revision_with_disabled(
        self,
        entries: Dict[str, McpServerEntry],
        disabled_servers: List[str],
    ) -> str:
        encoded = self._encode_servers(entries)
        disabled = [name for name in encoded if name in disabled_servers]
        content = {
            "mcpServers": encoded,
            "disabledMcpServers": sorted(disabled),
        }
    ...
    @staticmethod
    def _runtime_revision(servers: Dict[str, McpServerRuntime]) -> str:
        content = {
            "mcpServers": {
                name: servers[name].model_dump(
                    by_alias=True,
                    exclude_none=True,
                    mode="json",
                )
    ...
    def _load_disabled_servers(self, workspace_id: str) -> List[str]:
        ...
        path = self._local_file()
        data: object = read_json_file(path)
        if not isinstance(data, dict):
            logger.debug("[MCP Toggle] ~/.claude.json is not a dict")
            return []
        projects = data.get("projects")
        if not isinstance(projects, dict):
            logger.debug("[MCP Toggle] projects is not a dict")
            return []
        ...
        for key in candidates:
            ...
            if isinstance(project_block, dict):
                ...
                if isinstance(disabled, list):
                    ...
                    logger.info(
                        "[MCP Toggle] Found disabled servers in key '%s': %s",
                        key,
                        normalized_disabled,
                    )
                    return normalized_disabled

        logger.debug("[MCP Toggle] No disabled servers found")
        return []

    def _save_disabled_servers(
        self, workspace_id: str, disabled_servers: List[str]
    ) -> None:
        ...
        """
        self._update_project_block_field(
            workspace_id, "disabledMcpServers", disabled_servers
        )
    ...
    def _load_plugin_mcp_servers(
        ...
        plugin_mcp_payload: object = loader.load_plugin_mcp_servers(workspace_id)
        if not isinstance(plugin_mcp_payload, dict):
            logger.warning("Plugin MCP payload is not a dict")
            return {}
        ...
        all_servers: Dict[str, McpServerRuntime] = {}

        for plugin_id, servers in plugin_mcp_payload.items():
            if not isinstance(plugin_id, str) or "@" not in plugin_id:
                logger.warning("Invalid plugin MCP identifier: %r", plugin_id)
                continue
            plugin_name, marketplace_name = plugin_id.split("@", 1)

            if not isinstance(servers, dict):
                logger.warning(
                    "Plugin '%s' MCP servers is not a dict: %s",
                    plugin_id,
                    type(servers),
                )
                continue
            ...
            for server_name, server_config in servers.items():
                if not isinstance(server_name, str):
                    logger.warning(
                        "Plugin '%s' MCP server name is not a string: %r",
                        plugin_id,
                        server_name,
                    )
                    continue
                ...
                if not isinstance(server_config, dict):
                    logger.warning(
                        f"Plugin '{plugin_id}' server '{server_name}' config is not a dict: {type(server_config)}"
                    )
                    continue
                ...
                is_disabled = (
                    plugin_server_key in disabled_servers
                    or server_name in disabled_servers
                )

                # Convert to McpServerRuntime format
                try:
                    sanitized_config = sanitize_plugin_definition(server_config)
                    ...
                    logger.error(
                        f"Failed to validate MCP server '{server_name}' from plugin '{plugin_id}': {e}"
                    )
                    continue
    ...
    def _toggle_plugin_server_status(
        ...
        plugin_servers = self._load_plugin_mcp_servers(workspace_id)
        assert_revision(self._runtime_revision(plugin_servers), revision)
        ...
        return McpScopeResponse(
            workspaceId=workspace_id,
            scope=DocumentScope.PLUGIN,
            revision=self._runtime_revision({server_name: server_runtime}),
