...
class SettingsService:
        ...
        if scope is not None:
            ...
            return ClaudeCodeSettings(
                ...
                revision=self._settings_revision(
                    self._settings_file(workspace_id, scope)
                ),
            )
    ...
    def update_settings(
        ...
        assert_revision(self._settings_revision(file_path), payload.revision)
        ...
        if (
            ...
            or payload.field_provided("mode")
            or payload.field_provided("default_mode")
        ):
        ...
        if payload.field_provided("permissions"):
            state["permissions"] = (
                payload.permissions.model_dump() if payload.permissions else None
            )
        ...
        if payload.field_provided("enabled_plugins"):
            state["enabledPlugins"] = (
                payload.enabled_plugins if payload.enabled_plugins is not None else None
            )
        ...
        self._invalidate_provider_cache(workspace_id)
    ...
    def get_settings_revision(self, workspace_id: str, scope: DocumentScope) -> str:
        """Return the raw content revision for a scoped settings file."""

        return self._settings_revision(self._settings_file(workspace_id, scope))
    ...
    def update_raw_settings(
        ...
        file_path = self._settings_file(workspace_id, scope)
        assert_revision(self._settings_revision(file_path), revision)
        if content:
            try:
    ...
    # Internal Utilities ---------------------------------------
    @staticmethod
    def _settings_revision(file_path: Path) -> str:
        if not file_path.is_file():
            return compute_revision("")
        return compute_revision(file_path.read_bytes())
    ...
    def _aggregate_settings(self, workspace_id: str) -> ClaudeCodeSettings:
        ...
        aggregatable_fields: Dict[str, Tuple[Any, Callable[[Dict[str, Any]], Any]]] = {
            ...
            "include_co_authored_by": (
                None,
                lambda s: self._extract_bool(s, "includeCoAuthoredBy"),
            ),
            "disable_all_hooks": (
                None,
                lambda s: self._extract_bool(s, "disableAllHooks"),
            ),
            "enable_all_project_mcp": (
                None,
                lambda s: self._extract_bool(s, "enableAllProjectMcpServers"),
            ),
    ...
    def _extract_permissions(self, state: Dict[str, Any]) -> PermissionRules | None:
