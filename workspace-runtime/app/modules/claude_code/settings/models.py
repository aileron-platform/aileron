...
def _normalize_enabled_plugins(
    plugins: Mapping[str, Any] | Dict[str, bool] | None,
) -> Dict[str, bool]:
...
def _normalize_int(value: Any) -> Optional[int]:
    ...
    try:
        ...
        TypeError,
        ValueError,
    ) as exc:  # pragma: no cover - caught by validation process
...
class PermissionRules(BaseModel):
    ...
    allow: List[str] = Field(
        default_factory=list, description="Allowed operation rules"
    )
    deny: List[str] = Field(default_factory=list, description="Denied operation rules")
    ask: List[str] = Field(
        default_factory=list, description="Operation rules requiring confirmation"
    )
...
class ClaudeCodeSettings(BaseModel):
    ...
    mode: PermissionMode = Field(
        ...
        description="Currently effective permission mode",
    )
    default_mode: PermissionMode | None = Field(
        default=None,
        alias="defaultMode",
        description="Mode field for settings file storage",
    )
    ...
    api_key_helper: Optional[str] = Field(
        default=None,
        alias="apiKeyHelper",
        description="Helper script for generating API Key",
    )
    ...
    revision: str | None = Field(
        default=None,
        description="Content revision for the selected settings scope",
    )
    ...
    @field_validator(
        "enabled_mcpjson_servers", "disabled_mcpjson_servers", mode="before"
    )
    @classmethod
...
class ClaudeCodeSettingsUpdateRequest(BaseModel):
    ...
    revision: str | None = Field(
        default=None,
        description="Expected content revision for concurrency control",
    )
    ...
    @field_validator("allowed_mcp_servers", "denied_mcp_servers", mode="before")
    @classmethod
    def _validate_mcp_policy_lists(cls, value: Any) -> Optional[List[McpServerPolicy]]:
        if value is None:
            return None
        return _normalize_mcp_policy_list(value)
