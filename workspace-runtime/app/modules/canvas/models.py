...
class CanvasDetectResponse(BaseModel):
    ...
    runtime_status: CanvasRuntimeStatus | None = Field(
        default=None, alias="runtimeStatus"
    )
...
class CanvasHealthResponse(BaseModel):
    ...
    kind: CanvasKind | None = None
    manifest_status: CanvasManifestStatus | None = Field(
        default=None, alias="manifestStatus"
    )
    runtime_status: CanvasRuntimeStatus | None = Field(
        default=None, alias="runtimeStatus"
    )
...
class CanvasActionResponse(BaseModel):
    ...
    renderer_action_reason: str | None = Field(
        default=None, alias="rendererActionReason"
    )
    details: dict[str, Any] = Field(default_factory=dict)
...
class CanvasReviewElementTarget(BaseModel):
    ...
    parent_html_preview: str = Field(
        "", max_length=MAX_PREVIEW_LENGTH, alias="parentHtmlPreview"
    )
